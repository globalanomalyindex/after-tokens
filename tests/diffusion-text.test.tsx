...
describe('DiffusionText', () => {
  it('renders all tokenized words with data-state', () => {
    const { container } = render(<DiffusionText mode="crystal" trigger="manual">Speed beats slowness.</DiffusionText>)
  ...
  it('exposes editorial text without creating a live-region announcement', () => {
    render(<DiffusionText mode="crystal" trigger="manual">Hello world.</DiffusionText>)
  ...
  it('lets interactive results opt into one completion announcement', () => {
    render(<DiffusionText mode="crystal" trigger="manual" announce="on-complete">Hello world.</DiffusionText>)
    expect(screen.getByRole('status')).toBeEmptyDOMElement()
  })

  it('renders a motion-independent prototype status when requested', () => {
    render(<DiffusionText mode="crystal" trigger="manual" showStatus>Hello world.</DiffusionText>)
    expect(screen.getByText(/crystallize · authored · ready/i)).toHaveAttribute('data-prototype-status', 'ready')
  })
})
