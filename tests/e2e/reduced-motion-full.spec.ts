...
test.describe('reduced motion full audit', () => {
  ...
  test('all sections mount and every stage reaches its recording\'s end', async ({ page }) => {
    ...
    await expect(hook.locator('.settle-ink[data-released="true"]').first()).toBeVisible()
    ...
    const animating = await page.evaluate(() => {
      ...
      const own = Array.from(document.querySelectorAll('.settle-ambient, .settle-ink, .settle-candidate, .settle-passage, .settle-floor')).filter((el) => named(getComputedStyle(el).animationName)).length
