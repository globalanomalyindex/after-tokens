...
test('original demo gallery precedes the closing slide and scrolls within the page', async ({ page }) => {
  ...
  await expect(gallery.locator('[data-gallery-source]')).toHaveCount(4)
  await expect(gallery.getByRole('radio', { name: 'spectrum', exact: true })).toHaveCount(4)
  await gallery.getByRole('radio', { name: 'spectrum', exact: true }).first().click()
  await expect(gallery.locator('[data-brand="spectrum"]')).toHaveCount(1)
