...
an independent web and motion design study of adaptive skeleton loading and how generated text reaches a reader.
...
> **the current direction.** one rounded capsule divides into a breathing field whose rough shape follows available content. local cells make room for one another; an occasional glimmer coordinates the field. when text meets its release policy, the bubbles visible at that instant reshape toward its real word groups as the ink arrives. only the new batch settles. the same material supports word, sentence, paragraph and whole-answer release; earlier readable text stays in place.
...
waiting cells have no word identity. current committed non-end fragments can estimate total space, but only the contiguous prefix can guide coarse row lengths because a gap makes later line positions unknown. a genuinely received revisable snapshot can instead supply its current shape. font advances and explicit newlines become numeric occupancy budgets, quantized in 4% steps and capped at fourteen rows. a nonzero visual budget uses an authored 24% minimum footprint, while a known blank row stays empty. these budgets are not exact word wrapping, final formatting, confidence or progress. a final-only source starts with a neutral five-row allocation.

fourteen persistent rows have seeded identities. two are steady full rows; the others use three to six cells with independently phased 4–7 second shape cycles and 4.2–7 second breaths at neutral tempo. each cycle creates a new arrangement. a shared eight-second glimmer supplies an occasional coordinated event. the field preserves its left edge and typographic row rhythm while right edges can respond to current content. these authored variations are activity, not a visualization of decoder stages.
...
solid cell bodies and a faint gradient glimmer draw on existing loading conventions. the [CSS Script reference](https://www.cssscript.com/skeleton-loader-placeholder/) points to zalog's loader, whose source uses solid rows and a traveling gradient; [MUI](https://mui.com/material-ui/react-skeleton/) already provides pulse, wave and static variants. I treat these as prior art. the contribution is the source-limited geometry and material handover explored here, not the invention of a rounded skeleton.
...
1. future final text, word widths and formatting are unavailable to the waiting renderer. only current eligible source data can guide approximate space.
...
6. a revisable source can send snapshots; only an explicitly final one reaches the protected page. the current candidate may guide approximate geometry. later revisions preserve the prior version and require review and apply.
...
common-fate studies provide context for coordinated change, not a rule that every row must pulse together. this version deliberately varies local rhythms while keeping one occasional glimmer synchronized. that may make repetition less obvious, but could increase busyness; the reader study must test both. apple's fluid-interface and spring demonstrations inform positional continuity and restraint. the CSS treatment is neither Apple's glass material nor a physical spring. live-caption research motivates stable text after arrival; it does not validate fading or moving newly arriving letters.
...
[the current v6 handoff](docs/responsive-skeleton-v6-handoff-2026-09-09.md) records the source contract, motion parameters, architecture, code instructions, research justification and acceptance gates. it separates implementation behavior from reader hypotheses. the [v5 research record](docs/anchored-skeleton-research-2026-09-09.md) preserves the earlier material and source review. earlier feathered motion, continuous ink and interval geometry remain historical alternatives.

## verification and historical evidence

the current material is `responsive-cell-skeleton-v6`. verification of this revision must use its new contract: every policy uses released passages; only the new batch crosses from bubbles into text; old text remains stationary; current-content geometry does not consult future answers. fresh checks passed: **363 unit/component tests, 69 browser checks across three profiles, 39 current harness guards, lint and a production build with TypeScript**. eight [v6 observations](docs/responsive-skeleton-motion-validation-2026-09-12.json) preserve exact output and separate near-full opacity from settled access. the [12 September release record](docs/responsive-skeleton-v6-release-verification-2026-09-12.md) gives the evidence and its limits.

the archived `adaptive-cell-skeleton-v5` revision at `06da788390549574949c7cdcd938c3669820f214` passed **347 unit/component cases, 54 browser checks across three profiles and 44 measurement/archive guards**, plus lint, TypeScript and both hosting builds. its eight [instrumented observations](docs/anchored-skeleton-motion-validation-2026-09-09.json) retain exact output and **7,330 post-settle glyph samples with zero displacement**. its two [passive captures](docs/anchored-skeleton-showcase-capture-2026-09-09.json) remain archived as historical v5 examples. the website now presents [fresh v6 recordings](docs/responsive-skeleton-showcase-capture-2026-09-12.json), separately captured at narrow and wide browser sizes.
...
the [written case study](docs/case-study.md), [research note](docs/research-note.md), [earlier motion handoff](docs/motion-direction-2026-09-09.md) and [original release specification](docs/superpowers/specs/2026-09-07-settle-design.md) preserve the development record. the v6 handoff describes the current material; the anchored documents preserve v5. the earlier 268 tests, 42 browser checks, eight observations and 9,667 matched final-glyph samples belong to the feathered revision `05914c1`, recorded in its [historical handoff](docs/ambient-handoff-2026-09-09.md) and [measurement report](docs/ambient-motion-validation-2026-09-09.json). they are not reused as validation of the solid bars. no reader benefit or physical iPhone behavior has been measured.
