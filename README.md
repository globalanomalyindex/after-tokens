...
5. out-of-order text appears only after the page, never inside it: an open position is reserved space, and under every position runs a band of light, lit where the position is open, just over half as strong under a draft, and dissolving under a word as it lands, with a slow flock hovering over the open positions; every position is a reel, so a guess the model drops rolls up and out as the next rolls in beneath it; a confident guess is drawn as a draft in a ghost that sharpens with its probability; a committed piece of a word is drawn as the piece it is; a line break, committed or confidently guessed, is drawn as a break; and a word is written where it will stand, in the secondary ink, only after every piece of it has committed, rolling in last and building in place letter by letter from the middle of the word outward, or landing where it is when its own draft already had it right. nothing travels across the answer, and no build precedes a commitment.
...
10. reduced motion removes the breath, the bloom, the onset, the reel's roll, the build, the drafts' lift and breath, the stream's shimmer and its flock, and the set's press, glint and glow, and nothing else; rows stand where they are, the band stands still, the page's ink arrives at once, and words are drawn sharp at once; what is drawn, and when, is identical.
