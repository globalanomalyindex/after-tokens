...
- `compact/<prompt>__<config>.json`: what the site loads. one object per trajectory:
  ...
  - `drafts[]`: the model's drafts, one list per denoising step of `[pos, piece, p]` entries: the provisional argmax piece for a position it has not committed yet and that guess's max-softmax probability, quantized to 0.05. an entry is recorded when a position's draft first clears the record floor (0.2), when its text changes while above it, or when its probability moves by 0.05 or more; `[pos, "", 0]` withdraws one; a commitment at a position ends its draft implicitly. the site shows a draft only at or above `PROVISIONAL_FLOOR` (0.25) and keeps showing it while its text holds. produced by `scripts/derive-drafts.py`, which also writes `derived/drafts.json`
  - `spins[]`: the reel below the floor, one list per denoising step of `[pos, piece, p]` entries: recorded whenever an open position's provisional argmax changes at all, at any probability (the first step records every position), with that step's max-softmax probability quantized to 0.05. a commitment at a position ends its spin implicitly. the site draws a spin only as a smear blurred past reading; the same guess is a draft once it clears the floor. produced by `scripts/derive-drafts.py`
...
- `derived/drafts.json`: what the drafts do over every recording with at least 8 content tokens. `shown` is what the surface draws (the recorded entries under the reducer's rule at the display floor, accuracy by decoded text): how often a draft is visible, how often it is the token that later commits, how many steps before commit it first shows, how often it never changes again; per sampler, with the visible share by decile of the run. `byConfig` adds the neighbor lift: how much a commitment raises the max-probability of the adjacent open positions on the next step, against every other open position. `byFloor` explores the raw per-step probabilities at other floors. checked against the compact files by `tests/settle/drafts.test.ts`
...
every run gets one verdict from `scripts/gen-trace-index.mjs` (`AUDIT_RULE`), written into `TRACE_META[id].audit`: `complete` (the model emitted its own end-of-sequence token and the answer ends in terminal punctuation), `looped` (`LOOP_RULE`: a 2- to 8-word phrase repeated at least three times covering at least 30% of the words), `short` (under 8 content tokens), `empty` (no content token), or `cut`. tallies: `lowconf-b32` 16 complete, 4 looped (`heron-poem`, `lighthouse-haiku`, `sky-blue`, `capital`; one phrase of up to twenty words repeated 5 to 19 times over 72% to 88% of the words, a known failure of greedy decoding in a small model); `random-b32` 20 complete; `lowconf-b128` 11 complete, 6 short, 3 empty. the files are untouched and every run stays in the statistics it qualifies for. `curated.json` is a hand audit on top of that rule: the 18 runs that read as complete, coherent answers (`keep`), and a reason for each of the other 42 (`excluded`). the site's research stage shows only the kept runs, with the verdict beside the stage, and its product demos replay a run's order, timing, and confidence over pre-written words rather than the model's prose.
