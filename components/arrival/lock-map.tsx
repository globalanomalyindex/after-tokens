...
// that start where the gist is and close one another. When the map enters
// the viewport it draws itself in lock order over about a second, so the
// figure replays the arrival it describes.

export const LOCK_MAP_DRAW_MS = 1100
...
type Props = {
  ...
  nuclei?: Set<number>
  compact?: boolean
  /** milliseconds before the draw begins once in view, for a stagger across a figure */
  delay?: number
  className?: string
  style?: CSSProperties
}

export function LockMap({ atoms, locks, total, label, accent = false, nuclei, compact = false, delay = 0, className = '', style }: Props) {
  ...
  const span = Math.max(1, lastLock - firstLock)
  return (
    <div ref={ref} className={className} style={{ ...style, ['--lm-delay' as string]: `${delay}ms` } as CSSProperties} data-in={inView ? 'true' : 'false'}>
      <svg className="lock-map" viewBox={`0 0 ${W} ${H}`} role="img" aria-label={label ? `lock map, ${label}` : 'lock map'}>
        ...
        {atoms.map((a, i) => {
          ...
          // each mark appears at its share of the draw, in lock order
          const k = ((t - firstLock) / span).toFixed(3)
          const markStyle = { ['--k' as string]: k, ['--o' as string]: (0.35 + 0.65 * sal).toFixed(2) } as CSSProperties
          return isNucleus ? (
            <circle key={i} className="lm-mark" style={markStyle} cx={x(i)} cy={y(t)} r={3.2} fill="none" stroke={ink} strokeWidth={1.4} />
          ) : (
            <circle key={i} className="lm-mark" style={markStyle} cx={x(i)} cy={y(t)} r={1.9} fill={ink} />
          )
        })}
        {label && (
