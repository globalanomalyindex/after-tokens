...
const FRAMES = [
  {
    ...
    alt: 'User-supplied Gemini Diffusion frame 14: the answer panel contains a partial solution and blue changing fragments; the media label reads Real time output.',
  },
  {
    id: '035',
    label: '35 · revising',
    ...
    alt: 'User-supplied Gemini Diffusion frame 35: a seven-step math draft has blue changes in several rows and inconsistent top and bottom results; the media label reads Slowed down output.',
  },
  {
    id: '045',
    label: '45 · settled image',
    caption: 'The final supplied image shows a complete-looking solution without salient blue changes. A still image cannot establish an API completion event or an irreversible commitment.',
    alt: 'User-supplied Gemini Diffusion frame 45: seven numbered steps and a final expression are visible in white, with the media label Slowed down output.',
  },
]
    ...
    <div className="rule mt-12 md:mt-16 pt-8" aria-labelledby={titleId} data-google-reference>
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] mb-7">
        ...
        <p className="text-base leading-relaxed max-w-[64ch]" style={{ color: 'var(--ink-2)' }}>
          Google describes Gemini Diffusion as refining blocks of text. In this supplied reference, several parts of a math solution change while the page takes shape. That motivates a field that can respond to available content without making each unstable word a reading target. It does not establish that a real integration exposes drafts, final layout or token commitments.{' '}
      ...
      <p className="readout leading-relaxed mt-4 max-w-4xl" style={{ color: 'var(--muted)' }}>
        user-supplied JPEG sequence · original frames, selectable for inspection · the media switches between “Real time output” and “Slowed down output.” these are separate labeled sequences, without timestamps or an inferred frame rate.{' '}
