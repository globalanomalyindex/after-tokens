...
export function SettleAnswer({
  state, runId = 'default', voice: voiceProp, forming: formingProp, preview, field, status = true,
  haptics = false, motion = true, paused = false, onApplyRevision,
  ...
  const items = useMemo<DisplayItem[]>(() => carve({ ...state, releasedLength: 0 }).flatMap((item) => {
  ...
  useFormationLayout(rootRef, state, visible, enabled && !paused && inView && documentVisible, voice, runId)
  ...
  return (
    <div ref={rootRef} className={`settle ${className}`} data-status={state.status}
      ...
      <div className="settle-page" role="region" aria-label={label} tabIndex={state.previousPassages ? 0 : undefined}>
        {snapshot ? state.passages.map((p) => <span key={p.id} className="settle-passage">{p.text}</span>) : visible.map((item) =>
          <Cell key={`${runId}:v${state.version}:${item.position}`} item={item} released={state.releasedLength} offset={offsets.get(item.position)} limit={mode === 'carve' ? Infinity : Math.max(0, (mode === 'held' ? state.releasedLength : state.wordSafeLength) - (offsets.get(item.position) ?? 0))} />)}
        {!state.prefix && !visible.length && empty !== undefined && <span className="settle-empty" aria-hidden="true">{empty}</span>}
      </div>
