...
import { ARRIVAL, FINDINGS, FINDINGS_HEADING, LEAD, LIMITS, LLADA, ORDER_DRAWN } from '@/lib/traces/findings'
...
type Legibility = 'raw' | 'ordered'
const LEGIBILITY_ITEMS: { id: Legibility; label: string }[] = [
  { id: 'raw', label: 'as committed' },
  { id: 'ordered', label: 'reading order' },
]

const EYEBROW = 'label mb-3'
const EYEBROW_STYLE = {} as const

export function SectionSampler() {
  ...
  const [legibility, setLegibility] = useState<Legibility>('raw')
  ...
  const selectLegibility = useCallback((id: string) => setLegibility(id as Legibility), [])
  ...
  return (
    <Section id="sampler" title="What a real sampler does">
      ...
      <p className="standfirst max-w-3xl">{LEAD}</p>

      <div className="mt-16 md:mt-24">
        <div className={EYEBROW} style={EYEBROW_STYLE}>A recorded answer, replayed
        </div>
        <p className="text-base leading-relaxed max-w-3xl mb-8" style={{ color: 'var(--ink-2)' }}>
          The pending words show the model&rsquo;s own guess only when it clears a probability of {PROVISIONAL_FLOOR}; below that the guess is the corpus prior and would read &ldquo;the&rdquo; in every slot, so the slot shows noise instead. Under the default sampler the answer arrives nearly left to right because its block schedule says so. Switch the sampler to no blocks to watch the schedule-free order the grammar grows from: a few anchors, a long stretch in which the model holds its beliefs while the answer&rsquo;s length settles, then the words. Switch legibility to reading order to watch the two-channel reveal: every commit still ghosts the moment it lands, and inside each phrase the words turn crisp from left to right, the sampler&rsquo;s earliest commit in the phrase kept as the one crisp anchor.
        </p>

        <div className="mb-6">
          <div className={EYEBROW} style={EYEBROW_STYLE}>Pick a prompt
          </div>
          <PromptPicker prompts={promptItems} activeId={activePromptId} onSelect={selectPrompt} layout="compact" />
          <p className="mt-3 readout leading-relaxed max-w-2xl" style={{ color: 'var(--muted)' }}>
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,1fr)_360px] items-start">
          <div>
            ...
            {activeAudit && activeAudit.verdict !== 'complete' && (
              <p
                className="mt-3 readout leading-relaxed max-w-2xl"
          ...
          <aside className="flex flex-col gap-6">
            <div>
              <div className={EYEBROW} style={EYEBROW_STYLE}>This run, drawn live
              </div>
              <div
            ...
            <div className="flex flex-col gap-3">
              ...
              <ToggleRail label="Legibility" items={LEGIBILITY_ITEMS} activeId={legibility} onSelect={selectLegibility} />
              <p className="readout leading-relaxed" style={{ color: 'var(--muted)' }}>
                shaped spends the replay where the words are: a tail-only step plays in {SHAPED_TAIL_STEP_MS}{' '}ms, a step that commits a word in {SHAPED_CONTENT_STEP_MS}{' '}ms. the order is untouched. in reading order, legibility trails commitment by a median of {ARRIVAL.recorded.all.lagMedianMs}{' '}ms on the words that wait, {Math.round(ARRIVAL.recorded.all.waitedShare * 100)}% of them, and no phrase ever reads out of order.
              </p>
            </div>
      ...
      <div className="mt-16 md:mt-24">
        <div className={EYEBROW} style={EYEBROW_STYLE}>The order, drawn
        </div>
        {activeTrace && (
          <UnmaskMap
            trace={activeTrace}
      ...
      <div className="mt-16 md:mt-24">
        <div className={EYEBROW} style={EYEBROW_STYLE}>What sixty trajectories show
        </div>
        ...
        <div className="mt-10 max-w-3xl">
          <div className={EYEBROW} style={EYEBROW_STYLE}>Corroboration at 8B
          </div>
      ...
      <div className="mt-16 md:mt-24 max-w-3xl">
        <div className={EYEBROW} style={EYEBROW_STYLE}>Scope and limits
        </div>
        <p className="text-base leading-relaxed">{LIMITS}</p>
        <p className="mt-4 readout leading-relaxed" style={{ color: 'var(--muted)' }}>
          Data and method:{' '}
          <a
