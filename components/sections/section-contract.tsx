...
const RULES: string[] = [
  ...
  'out-of-order text appears only after the page, never inside it. an open position is reserved blank space under a stream of light; the source&rsquo;s own guess for it, above a floor, is drawn as a draft and never as a word, and every position is a reel: a guess the source drops rolls up and out as the next rolls in beneath it; a committed piece is drawn as a piece; a word rolls in last and snaps where it will stand, in the secondary ink, only after every piece of it has committed. nothing committed is drawn as a guess, nothing guessed is drawn as committed, and no guess reaches the page. nothing travels across the answer.',
  ...
  'reduced motion removes the breath, the bloom, the build, the reel&rsquo;s roll, the stream&rsquo;s shimmer and its flock, and the set, and changes nothing else.',
]
