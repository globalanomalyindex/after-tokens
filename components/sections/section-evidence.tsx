...
const LEDGER: Row[] = [
  ...
  { mechanism: 'elastic character', source: 'Apple, Meet Liquid Glass, WWDC 2025', sourceUrl: 'https://developer.apple.com/videos/play/wwdc2025/219/', says: 'Liquid Glass combines fluid motion with a material reserved for navigation and controls above content. Reduced Motion disables its elastic properties.', does: 'recommend glass skeletons, validate these timing values, or report benefits for generated text.', design: 'borrow restrained movement in an opaque content placeholder. keep row left edges anchored, with full bars between a few independently reshaping larger cells. CSS easing supplies the character, not a physical spring or a velocity-continuity guarantee.', verdict: 'hypothesis' },
  { mechanism: 'reserving room', source: 'web.dev, Optimize Cumulative Layout Shift, updated 2025', sourceUrl: 'https://web.dev/articles/optimize-cls', says: 'reserving space before late content arrives can avoid unexpected shifts; unknown content size creates a tradeoff between empty space and movement.', does: 'make a growing height animation free of layout work or establish a benefit for generated text.', design: 'use a coarse high-water estimate, coalesce source-only updates and measure the final eligible page. report local glyph stability separately from viewport movement. a smooth size change is not a zero-CLS claim.', verdict: 'guardrail' },
  ...
  { mechanism: 'common fate', source: 'Chalbi and colleagues, 2020', sourceUrl: 'https://arxiv.org/html/1908.00661', says: 'an online study with 100 people found grouping from coordinated visual changes, including luminance and size; later visualization tasks showed context dependence.', does: 'show that synchronized bars improve reading or establish a best breathing period.', design: 'retain a shared glimmer across independently breathing and reshaping rows. test whether coordination helps the field feel coherent or increases distraction. the comparison tests the combined treatment, not a benefit established by common fate.', verdict: 'hypothesis' },
...
const STUDY: { dt: string; dd: string }[] = [
  { dt: 'motion comparison', dd: 'still, breathe and reshape use the same source, selected policy and bubble-to-word handover; the current previews default to each sentence. all conditions share the same coarse current-content sizing rule; individual bar widths remain authored. reshape combines an opening capsule, independent local reshaping and shared glimmer; opening geometry and moving ink differ. measure visual availability and size correction separately. test the handover itself in a separate controlled ablation.' },
  { dt: 'availability, separately', dd: 'compare earlier-sentence access with the whole answer at each policy&rsquo;s real release time. report presentation hold and time to a correct usable answer. this comparison cannot isolate a motion effect.' },
    ...
    <Section id="evidence" title="What is known">
      ...
      <div className="mt-16 md:mt-24 rule pt-8">
        ...
        <div className="grid gap-8 md:grid-cols-2 max-w-3xl mt-7">
          {[
            { id: 'sentence-mobile', title: 'sentences, on a narrow page', detail: '390 × 844 · weather metaphor · each sentence', width: 390, height: 844 },
            { id: 'answer-wide', title: 'one answer, with more room', detail: '1380 × 900 · same source · whole-answer comparison', width: 1380, height: 900 },
          ].map((recording) => (
            <figure key={recording.id} className="m-0 min-w-0">
              <video
                ...
                aria-label={`V8 browser capture: ${recording.title}`}
                aria-describedby={`showcase-${recording.id}-caption`}
                data-showcase-video={recording.id}
                src={`${assetBase}/study/growing-skeleton-v8-showcase-${recording.id}.webm`}
                poster={`${assetBase}/study/growing-skeleton-v8-showcase-${recording.id}.png`}
              >
                <a href={`${assetBase}/study/growing-skeleton-v8-showcase-${recording.id}.webm`}>Open the browser recording</a>
              </video>
      ...
      <div className="mt-16 md:mt-24">
        ...
        <p className="text-base leading-relaxed max-w-[64ch] mb-8" style={{ color: 'var(--ink-2)' }}>
          the main literature review was refreshed on 9 september 2026; skeleton, spring and layout guidance was checked again on 12 september. source findings are separated from design hypotheses.
          the solid skeleton comparison and its sentence and whole-answer policies are a prototype; comfort, comprehension and satisfaction effects have not been measured.
      ...
      <div className="mt-16 md:mt-24 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] rule pt-8">
        <div>
          ...
          <p className="mt-3 text-base leading-relaxed max-w-[44ch]" style={{ color: 'var(--ink-2)' }}>
            the proposal is that fuller bars, gradual space reservation and the retained bubble-to-word handover make the wait feel coherent and continuous. it could be wrong: the pulse alone may work as well, the added motion may distract, or waiting for the complete answer may cost too much.
          </p>
