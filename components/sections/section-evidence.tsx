...
const LEDGER: Row[] = [
  ...
  { mechanism: 'fluid motion', source: 'Apple, Designing Fluid Interfaces, WWDC 2018', sourceUrl: 'https://developer.apple.com/videos/play/wwdc2018/803/', says: 'gesture demonstrations emphasize immediate response, interruption, spatial continuity and small changes between adjacent frames.', does: 'report a diffusion reader study, prescribe these bar timings or establish a dopamine response.', design: 'keep bar motion continuous inside one bounded area. the final answer is immediately readable while decoration dissipates; its tactile quality is a design hypothesis.', verdict: 'hypothesis' },
  ...
  { mechanism: 'animated transitions', source: 'Heer and Robertson, 2007', sourceUrl: 'https://idl.cs.washington.edu/files/2007-AnimatedTransitions-InfoVis.pdf', says: 'animated chart transitions helped object tracking and change estimation; simple staging was preferred, while complicated staging sometimes increased errors.', does: 'test text or supply an optimum duration for word motion.', design: 'test a coherent composition rather than independent token marks. the answer arrives together after source finality, while the final height change remains a separate geometry measurement.', verdict: 'hypothesis' },
  ...
  { mechanism: 'common fate', source: 'Chalbi and colleagues, 2020', sourceUrl: 'https://arxiv.org/html/1908.00661', says: 'an online study with 100 people found grouping from coordinated visual changes, including luminance and size; later visualization tasks showed context dependence.', does: 'show that synchronized bars improve reading or establish a best breathing period.', design: 'compare static, coherent and independent ambient bars with the same source events, answer timing and handover. grouping is the hypothesis, not the result.', verdict: 'hypothesis' },
...
const STUDY: { dt: string; dd: string }[] = [
  { dt: 'motion, isolated', dd: 'static bars, coherent ambient bars and independent ambient bars use the same source, whole-answer policy, release timestamp and final handover. match luminance, area and motion range where possible; report remaining differences.' },
  ...
  { dt: 'primary outcomes', dd: 'for motion: perceived fragmentation and fluidity, with activity understanding and false progress estimates. for availability: task accuracy and time to a correct usable answer, with source time recorded separately.' },
    ...
    <Section id="evidence" title="What is known">
      ...
      <div className="mt-16 md:mt-24">
        ...
        <p className="text-base leading-relaxed max-w-[64ch] mb-8" style={{ color: 'var(--ink-2)' }}>
          ...
          the ambient comparison and its whole-answer policy are a prototype; comfort, comprehension and satisfaction effects have not been measured.
      ...
      <div className="mt-16 md:mt-24 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] rule pt-8">
        <div>
          ...
          <p className="mt-3 text-base leading-relaxed max-w-[44ch]" style={{ color: 'var(--ink-2)' }}>
            the proposal is that fluid ambient bars make waiting feel more coherent, followed by an answer that arrives together. it could be wrong: independent motion may work as well, the wait may cost too much, or a polished wrong answer may be believed too easily.
          </p>
