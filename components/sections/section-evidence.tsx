...
const LEDGER: Row[] = [
  { mechanism: 'stability of text under the eye', term: 'change blindness', source: 'Liu and colleagues, CHI 2023 extended abstracts; Slattery, Angele and Rayner, 2011', sourceUrl: 'https://research.google/pubs/modeling-and-improving-text-stability-in-live-captions/', says: 'caption instability correlated with distraction, fatigue and lower reading comfort; stabilization with smoothing improved five of six ratings in a study of 123 people.', does: 'test diffusion text, isolate the benefit of animation, or establish a comprehension gain.', design: 'the whole-answer treatment avoids incremental readable-text reflow, then adds one small whole-answer settling motion. that arrival motion and the wait it adds are separate design choices; a reader benefit remains untested.', verdict: 'kept' },
  ...
  { mechanism: 'fluid motion', source: 'Apple, Designing Fluid Interfaces, WWDC 2018', sourceUrl: 'https://developer.apple.com/videos/play/wwdc2018/803/', says: 'gesture demonstrations emphasize immediate response, interruption, spatial continuity and small changes between adjacent frames.', does: 'report a diffusion reader study, prescribe these bar timings or establish a dopamine response.', design: 'keep bar motion continuous inside one bounded area. a final size fit has an authored 180 ms duration before complete text is visible; browser scheduling may add more. a small whole-answer settle and thin outline follow. its tactile quality is a design hypothesis.', verdict: 'hypothesis' },
  { mechanism: 'elastic character', source: 'Apple, Meet Liquid Glass, WWDC 2025', sourceUrl: 'https://developer.apple.com/videos/play/wwdc2025/219/', says: 'Liquid Glass combines fluid motion with a material reserved for navigation and controls above content. Reduced Motion disables its elastic properties.', does: 'recommend glass skeletons, validate these timing values, or report benefits for generated text.', design: 'borrow a restrained settling character in an opaque content placeholder. use a brief division introduction, then keep row envelopes fixed and confine the elastic response to smaller shapes inside three clusters. CSS keyframes approximate the character; they are not a physical spring.', verdict: 'hypothesis' },
  { mechanism: 'motion in the periphery', source: 'Bartram, Ware and Calvert, 2003', sourceUrl: 'https://scholars.unh.edu/ccom/979/', says: 'moving notification icons were readily detected; traveling and zooming cues were rated more distracting, and slow linear motion and slow blink less distracting.', does: 'test breathing words or prove that ambient motion improves reading.', design: 'keep ambient motion within the loading area and distinguish source finality from the brief presentation fit that may follow it. compare its distraction cost with static bars; reduced motion preserves source finality and bypasses the decorative final fit delay.', verdict: 'guardrail' },
  { mechanism: 'animated transitions', source: 'Heer and Robertson, 2007', sourceUrl: 'https://idl.cs.washington.edu/files/2007-AnimatedTransitions-InfoVis.pdf', says: 'animated chart transitions helped object tracking and change estimation; simple staging was preferred, while complicated staging sometimes increased errors.', does: 'test text or supply an optimum duration for word motion.', design: 'test the combined division introduction, pill appearance, neighbor redistribution, local settle and glimmer against a shared opacity pulse. the answer arrives together after source finality and any size fit; added visual delay, initial text travel and final height change are separate measurements.', verdict: 'hypothesis' },
...
const STUDY: { dt: string; dd: string }[] = [
  { dt: 'motion comparison', dd: 'still, breathe and reshape use the same source, whole-answer policy and final handover rules. actual visual availability and frame-size correction must be measured separately. reshape combines a division introduction, interior pill formation, neighbor redistribution, local settling and occasional glimmer. its initial presentation differs from the controls; visible area, ink and pill presence also change. this is a combined treatment.' },
  ...
  return (
    <Section id="evidence" title="What is known">
      ...
      <div className="mt-16 md:mt-24 rule pt-8">
        <h3 className="text-2xl md:text-3xl font-bold tracking-tight leading-tight mb-3">the material, in the browser</h3>
        <p className="text-base leading-relaxed max-w-[64ch]" style={{ color: 'var(--ink-2)' }}>
          two actual browser captures of the current treatment. the shorter answer needs little room; the longer output lets the field grow before the words arrive. both retain the model&rsquo;s original text, including its flaws.
        </p>
        <div className="grid gap-8 md:grid-cols-2 max-w-3xl mt-7">
          {[
            { id: 'mobile', title: 'a short answer', detail: 'sleep tips · the five-row allocation shrinks to the final answer' },
            { id: 'long', title: 'more room as it forms', detail: 'sky-blue explanation · the waiting area grows from five to ten rows' },
          ].map((recording) => (
            <figure key={recording.id} className="m-0 min-w-0">
              <video
                ...
                aria-label={`Passive browser capture: ${recording.title}`}
                aria-describedby={`showcase-${recording.id}-caption`}
                data-showcase-video={recording.id}
                src={`${assetBase}/study/anchored-skeleton-showcase-${recording.id}.webm`}
                poster={`${assetBase}/study/anchored-skeleton-showcase-${recording.id}.png`}
              >
                <a href={`${assetBase}/study/anchored-skeleton-showcase-${recording.id}.webm`}>Open the browser recording</a>
              </video>
        ...
        <p className="readout leading-relaxed max-w-4xl mt-5" style={{ color: 'var(--muted)' }}>
          390 × 844 chromium captures · 0.5× source inspection · untrimmed setup, scrolling and replay · no audio. these presentation recordings omit dense per-frame sampling; they are separate from the instrumented validation videos. browser recording still has overhead.{' '}
          <a className="underline underline-offset-4" href="https://github.com/globalanomalyindex/after-tokens/blob/main/docs/anchored-skeleton-showcase-capture-2026-09-09.json">capture provenance</a>
        </p>
        <p className="text-sm leading-relaxed max-w-3xl mt-5" style={{ color: 'var(--ink-2)' }}>
          In eight separate instrumented observations, final text stayed exact and was withheld until source finality. The intended arrival moved the answer by about 1.7 px across its full range, then 7,330 matched post-settle glyph samples showed no movement. These eight cases needed no delayed fit. Separately, forcing a long final answer into the initial five-row space added 195–204 ms before visibility in three browser trials. That measures an artificial fitting test, not model latency. Dense inspection adds workload; its diagnostic recordings cannot establish ordinary playback smoothness. Reader benefits and physical iPhone behavior remain untested.{' '}
          <a className="underline underline-offset-4" href="https://github.com/globalanomalyindex/after-tokens/blob/main/docs/anchored-skeleton-release-verification-2026-09-09.md">results and limits</a>
        </p>
      </div>
      ...
      <div className="mt-16 md:mt-24 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] rule pt-8">
        <div>
          ...
          <p className="mt-3 text-base leading-relaxed max-w-[44ch]" style={{ color: 'var(--ink-2)' }}>
            the proposal is that a dividing capsule, softly forming word-like shapes and occasional glimmer improve a familiar breathing skeleton. it could be wrong: the pulse alone may work as well, the added motion may distract, or waiting for the complete answer may cost too much.
          </p>
