...
const LEDGER: Row[] = [
  ...
  { mechanism: 'elastic character', source: 'Apple, Meet Liquid Glass, WWDC 2025', sourceUrl: 'https://developer.apple.com/videos/play/wwdc2025/219/', says: 'Liquid Glass combines fluid motion with a material reserved for navigation and controls above content. Reduced Motion disables its elastic properties.', does: 'recommend glass skeletons, validate these timing values, or report benefits for generated text.', design: 'borrow restrained movement in an opaque content placeholder. keep row left edges anchored while coarse content budgets can change their right edges; local cells redistribute space. CSS easing supplies the character, not a physical spring or a velocity-continuity guarantee.', verdict: 'hypothesis' },
  ...
  { mechanism: 'common fate', source: 'Chalbi and colleagues, 2020', sourceUrl: 'https://arxiv.org/html/1908.00661', says: 'an online study with 100 people found grouping from coordinated visual changes, including luminance and size; later visualization tasks showed context dependence.', does: 'show that synchronized bars improve reading or establish a best breathing period.', design: 'retain an occasional shared glimmer while local breaths and shapes use different rhythms. that deliberately varies coordination and may increase busyness. the comparison tests the combined treatment, not a benefit established by common fate.', verdict: 'hypothesis' },
...
const STUDY: { dt: string; dd: string }[] = [
  { dt: 'motion comparison', dd: 'still, breathe and reshape use the same source, whole-answer policy and bubble-to-word handover. current content can resize all conditions. reshape combines division, independently phased shape changes, local breathing and shared glimmer; opening geometry and moving ink differ. measure visual availability and size correction separately. test the handover itself in a separate controlled ablation.' },
  ...
  return (
    <Section id="evidence" title="What is known">
      ...
      <div className="mt-16 md:mt-24 rule pt-8">
        <h3 className="text-2xl md:text-3xl font-bold tracking-tight leading-tight mb-3">the material, recorded</h3>
        <p className="text-base leading-relaxed max-w-[64ch]" style={{ color: 'var(--ink-2)' }}>
          two browser recordings of the current v6 treatment: a narrow list and a wider explanation. the cells reshape toward eligible word groups as the text arrives. both retain the model&rsquo;s original answer, including its flaws.
        </p>
          ...
          {[
            ...
            <figure key={recording.id} className="m-0 min-w-0">
              <video
                ...
                aria-label={`V6 browser capture: ${recording.title}`}
                aria-describedby={`showcase-${recording.id}-caption`}
                data-showcase-video={recording.id}
                src={`${assetBase}/study/responsive-skeleton-showcase-${recording.id}.webm`}
                poster={`${assetBase}/study/responsive-skeleton-showcase-${recording.id}.png`}
              >
                <a href={`${assetBase}/study/responsive-skeleton-showcase-${recording.id}.webm`}>Open the browser recording</a>
              </video>
        ...
        <p className="readout leading-relaxed max-w-4xl mt-5" style={{ color: 'var(--muted)' }}>
          v6 chromium captures · 0.5× source inspection · untrimmed setup, scrolling and replay · no audio. these passive recordings omit per-frame measurement. posters are separate browser screenshots of the same material. recording still has overhead.{' '}
          <a className="underline underline-offset-4" href="https://github.com/globalanomalyindex/after-tokens/blob/main/docs/responsive-skeleton-showcase-capture-2026-09-12.json">capture provenance</a>
        </p>
        <p className="text-sm leading-relaxed max-w-3xl mt-5" style={{ color: 'var(--ink-2)' }}>
          Eight instrumented v6 observations preserved exact text and showed no early protected answer. Near-full opacity was first sampled 200–233 ms after source completion; settled, unoccluded readiness followed at 300–333 ms. These cases needed no fitting delay. Separate forced narrow-fit tests reached near-full opacity in 396–423 ms. Earlier-reading tests kept sampled glyphs stable relative to the page; the mobile page still moved within the viewport. These are browser observations, not model latency, reader benefits or physical iPhone results.{' '}
          <a className="underline underline-offset-4" href="https://github.com/globalanomalyindex/after-tokens/blob/main/docs/responsive-skeleton-v6-release-verification-2026-09-12.md">results and limits</a>
      ...
      <div className="mt-16 md:mt-24 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] rule pt-8">
        <div>
          ...
          <p className="mt-3 text-base leading-relaxed max-w-[44ch]" style={{ color: 'var(--ink-2)' }}>
            the proposal is that local shape variation and a bubble-to-word handover make a familiar skeleton feel more continuous. it could be wrong: the pulse alone may work as well, the added motion may distract, or waiting for the complete answer may cost too much.
          </p>
