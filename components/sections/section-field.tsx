...
const REGISTERS: { label: string; body: string; sample: React.ReactNode }[] = [
  { label: 'open slot', body: 'a position with no commitment yet: static, standing where a word will', sample: <span className="settle-carve"><span className="settle-slot" data-state="open" /> <span className="settle-slot" data-state="open" /> <span className="settle-slot" data-state="open" /></span> },
  { label: 'held slot', body: 'a piece of a word is in; the rest is out, so no letters are drawn', sample: <span className="settle-carve"><span className="settle-slot" data-state="held" /> <span className="settle-slot" data-state="held" /></span> },
  { label: 'carved word', body: 'every piece and its boundaries are in; it stands where it will, dim, out of order', sample: <span className="settle-carve"><span className="settle-cw">sunlight</span></span> },
  { label: 'forming word', body: 'joined the in-order prefix; brighter, waiting for its passage to close', sample: <span className="settle-forming"><span className="settle-fw">because of</span></span> },
  { label: 'on the page', body: 'released under the policy; full ink, still, selectable', sample: <span className="settle-passage"><span className="settle-w">The sky is blue.</span></span> },
  ...
  return (
    <Section id="field" title="What settles first">
      ...
      <p className="standfirst max-w-3xl">
        ...
        a slot of static while it is open, a held slot when a piece of a word is in, the word itself, dim, once every
        piece and its boundaries are in, and one end mark at the lowest position the sampler has marked as end, since an
        end token anywhere bounds the answer to before it. the zone is cut down from the tail as the model decides the
        length, words appear across it in the sampler&rsquo;s own order, and the page sweeps over the solved part. no
        letter is drawn that the source has not committed.
      </p>
      ...
      <div className="mt-12 md:mt-16 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] rule pt-8">
        <div>
          ...
          <p className="mt-3 text-base leading-relaxed max-w-[44ch]" style={{ color: 'var(--ink-2)' }}>
            a word&rsquo;s progress is its register. it is carried by the word, where the reader is looking, and by nothing else.
          </p>
        </div>
      ...
      <div className="mt-12 md:mt-16 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)]">
        ...
        <div className="text-base leading-relaxed max-w-[60ch]" style={{ color: 'var(--ink-2)' }}>
          <p>
            ...
            it costs nothing in truth, because every slot and every word is a fact the sampler already emitted.
          </p>
          <p className="mt-4">
          ...
          <p className="mt-4">
            ...
            people put on a result whether or not the result deserves it. so a slot is never a letter, a word is never a score, and the
            study measures whether a calmer surface gets believed too easily.
          </p>
          <p className="mt-4">
