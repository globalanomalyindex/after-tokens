...
const REGISTERS: { label: string; body: string; sample: React.ReactNode }[] = [
  { label: 'open', body: 'a position with no commitment yet: reserved blank space, about a token wide, standing where a word will', sample: <span className="settle-zone settle-legend"><span className="settle-slot" data-state="open" /><span className="settle-slot" data-state="open" /><span className="settle-slot" data-state="open" /></span> },
  { label: 'resolving', body: 'a piece of a word is in; the rest is out. its place glimmers, and no letters are drawn', sample: <span className="settle-zone settle-legend"><span className="settle-slot" data-state="held" /><span className="settle-slot" data-state="held" /></span> },
  { label: 'written', body: 'every piece and its boundaries are in. the cursor glides there and the word opens from the space it reserved, in the secondary ink, waiting for its sentence', sample: <span className="settle-zone settle-legend"><span className="settle-cw" style={{ animation: 'none' }}>sunlight scatters</span></span> },
  ...
  { label: 'the cursor', body: 'a circle on the line, the brand\u2019s color. it goes where the source just committed, sits after the word it wrote, breathes while the source works, and fades when the answer is done', sample: <span className="settle-legend settle-legend-cursor"><span className="settle-cursor" data-state="active" style={{ position: 'static', margin: 0, display: 'inline-block', verticalAlign: 'middle' }} /></span> },
]
  ...
  return (
    <Section id="field" title="What settles first">
      ...
      <p className="standfirst max-w-3xl">
        ...
        space about a token wide. when the sampler commits a piece of a word, its place glimmers. when every piece is in,
        the cursor glides there and writes the word in a secondary ink, opening it from the space it reserved so the
        line slides rather than jumps. when a sentence closes the cursor sweeps to the end of the page and the
        page&rsquo;s ink settles through the sentence. the cursor moves out of order because the model does; the space
        is carved down from the tail as the model decides the length. nothing is written that the source has not
        committed.
      </p>
      ...
      <div className="mt-12 md:mt-16 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] rule pt-8">
        <div>
          ...
          <p className="mt-3 text-base leading-relaxed max-w-[44ch]" style={{ color: 'var(--ink-2)' }}>
            a word&rsquo;s progress is its ink: a glimmer where a piece of it is in, its own letters in the secondary ink once the cursor
            has written it, then the page&rsquo;s. it is carried by the word, where the reader is looking, and by nothing else. the
      ...
      <div className="mt-12 md:mt-16 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)]">
        ...
        <div className="text-base leading-relaxed max-w-[60ch]" style={{ color: 'var(--ink-2)' }}>
          <p>
            ...
            it costs nothing in truth, because every reserved space is a real open position, every glimmer a real committed piece, and
            every word a fact the sampler already emitted. the margin keeps a
          ...
          <p className="mt-4">
            ...
            people put on a result whether or not the result deserves it. so a glimmer never spells, a word is never a score, and
