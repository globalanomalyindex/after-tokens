...
const REGISTERS: { label: string; body: string; sample: React.ReactNode }[] = [
  { label: 'open', body: 'a position with no commitment yet: a short run of noise glyphs that cycle and never spell anything, standing where a word will', sample: <span className="settle-carve"><span className="settle-slot" data-state="open">qvx</span><span className="settle-slot" data-state="open">hmk</span><span className="settle-slot" data-state="open">tzr</span></span> },
  { label: 'held', body: 'a piece of a word is in; the rest is out, so the noise firms and still spells nothing', sample: <span className="settle-carve"><span className="settle-slot" data-state="held">bwn</span><span className="settle-slot" data-state="held">gsd</span></span> },
  { label: 'available', body: 'every piece and its boundaries are in. the word&rsquo;s letters resolve out of the noise, left to right, and it stands where it will in the secondary ink, waiting for its sentence', sample: <span className="settle-carve"><span className="settle-cw">sunlight scatters</span></span> },
  ...
  { label: 'end', body: 'the lowest committed end token; the answer ends at or before it, so nothing past it is drawn', sample: <span className="settle-carve"><span className="settle-slot" data-state="end">¶</span></span> },
]
  ...
  return (
    <Section id="field" title="What settles first">
      ...
      <p className="standfirst max-w-3xl">
        ...
        a run of noise glyphs while it is open, firmer noise when a piece of a word is in, the word itself once every
        piece and its boundaries are in, its letters resolving out of the noise in a secondary ink, and one end mark at
        the lowest position the sampler has marked as end, since an end token anywhere bounds the answer to before it.
        the noise is carved down from the tail as the model decides the length, words resolve across it in the
        sampler&rsquo;s own order, and when a sentence closes the page&rsquo;s ink settles through its letterforms, bottom
        to top, in one movement. noise is never content: it cycles, it never spells, and a word&rsquo;s letters come out of
        it only after the source has committed every piece of the word.
      </p>
      ...
      <div className="mt-12 md:mt-16 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] rule pt-8">
        <div>
          ...
          <p className="mt-3 text-base leading-relaxed max-w-[44ch]" style={{ color: 'var(--ink-2)' }}>
            a word&rsquo;s progress is its ink: noise, then its own letters in the secondary ink, then the page&rsquo;s. it is carried by the
            word, where the reader is looking, and by nothing else. the
      ...
      <div className="mt-12 md:mt-16 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)]">
        ...
        <div className="text-base leading-relaxed max-w-[60ch]" style={{ color: 'var(--ink-2)' }}>
          <p>
            ...
            it costs nothing in truth, because every run of noise stands for a real open position and every word is a fact the sampler
            already emitted. the margin keeps a
          ...
          <p className="mt-4">
            ...
            page, the same states, the field&rsquo;s compact form. as noise becomes words the zone reflows, and everything that stays
          ...
          <p className="mt-4">
            ...
            people put on a result whether or not the result deserves it. so the noise never spells, a word is never a score, and
