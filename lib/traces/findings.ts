...
import draftsReport from '@/data/traces/derived/drafts.json'
import settleReport from './settle.json'
...
// ===== The causal audit and the cost of settling ===========================
// Generated by tests/settle/report.test.ts (pnpm traces:settle) into
// lib/traces/settle.json. Every number is a property of the reducer on this
// corpus, on a stated clock and denominator. None is a reader outcome.

export type PolicySummary = {
  traces: number
  nonemptyTraces: number
  exactFinalOutputs: number
  precommitExposure: number
  characters: number
  medianFirstPassageAt: number | null
  medianCompletionAt: number | null
  meanExtraHold: number | null
  medianExtraHold: number | null
  meanWordSafeLag: number | null
  medianMaxQueued: number | null
  maxQueued: number
  medianPassages: number | null
  medianPassageChars: number | null
  medianFormingShare: number | null
}
export type SettlePolicyKey = 'word' | 'sentence' | 'paragraph'
export type SettleReport = {
  note: string
  all60: Record<SettlePolicyKey, { uniform: PolicySummary; recorded: PolicySummary }>
  curated18: Record<SettlePolicyKey, { uniform: PolicySummary; recorded: PolicySummary }>
  audit: Record<'all60' | 'curated18', { words: number; multiStepWords: number; matchedAtFirst: number; tailStatEarly: number; traces: number }>
}
...
export const CAUSAL = {
  ...
  words: SETTLE.audit.all60.words,
  multiStepShare: SETTLE.audit.all60.multiStepWords / SETTLE.audit.all60.words,
  /** of those, how many already matched the model's provisional guess at the first commit */
  matchedAtFirst: SETTLE.audit.all60.matchedAtFirst,
  curatedMultiStepWords: SETTLE.audit.curated18.multiStepWords,
  curatedWords: SETTLE.audit.curated18.words,
  /** traces whose stored tail statistic precedes the step at which the answer's length was causally known */
  tailStatEarly: SETTLE.audit.all60.tailStatEarly,
  traces: SETTLE.audit.all60.traces,
  /** the median within-phrase inversion share the old reading-order transform still left, from arrival.json */
  inversionsLeft: 0.056,
} as const

// ===== The drafts ==========================================================
// Generated by scripts/derive-drafts.py into data/traces/derived/drafts.json,
// which also writes the per-step drafts into the compact traces the site
// loads. Computed over content positions of every recording with at least
// eight content tokens; a pair is one open content position at one step.
// `shown` is what the surface draws: the recorded entries under the reducer's
// rule at the display floor (a guess shows at or above the floor and keeps
// showing while its text holds), accuracy by decoded text. `byFloor` explores
// the raw per-step probabilities at other floors, accuracy by token id.
// tests/settle/drafts.test.ts checks these against the compact data.
// None of these numbers is a reader outcome.

export type DraftConfigKey = 'lowconf-b32' | 'random-b32' | 'lowconf-b128'
export type DraftShare = {
  /** the share of open-position steps at which a draft is drawn */
  visibleShare: number
  /** the share of drawn drafts that are the token the position later commits */
  accuracy: number
  /** how many steps a position's draft shows before it commits */
  medianPolishSteps: number
  visibleShareByDecile: number[]
}
export type DraftsReport = {
  model: string
  /** the probability at or above which the capture records a draft */
  recordFloor: number
  /** the probability at or above which the surface draws one */
  displayFloor: number
  ...
  shown: {
    visibleShare: number
    accuracy: number
    /** the share of steps with at least one draft on screen */
    stepsWithDraftShare: number
    medianPolishSteps: number
    meanPolishSteps: number
    /** the share of drafts that never change again once they show */
    neverChangedShare: number
    byConfig: Record<DraftConfigKey, DraftShare>
  }
  byFloor: Record<string, {
    ...
    meanPolishSteps: number
    neverChangedShare: number
    meanFlipsAfterClearing: number
    wordInitialShare: number
  }>
  byConfig: Record<DraftConfigKey, {
    runs: number
    visibleShare: number
    accuracy: number
    medianPolishSteps: number
    medianFirstDraftStep: number
    visibleShareByDecile: number[]
    /** the mean rise in an open content position's max probability at the step
     *  after a commitment beside it, against every other open position */
    neighborLift: number
    neighborLiftN: number
    otherLift: number
    otherLiftN: number
  }>
}
export const DRAFTS = draftsReport as unknown as DraftsReport
