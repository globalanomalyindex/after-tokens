...
10. Reduced motion removes the breath, the bloom, the onset, the reel's roll, the bounce, the build, the drafts' lift and breath, and the set's press, glint and glow, and nothing else. Rows stand where they are, a smear stands where it is at its weight, the page's ink arrives at once, and written words and their letters are drawn sharp at once. What is drawn, and when, is identical.
...
The reserved space is the message's shape, and it is drawn first: every open position is about a token wide. Every position of the zone is one element, whatever it happens to show: reserved space, a guess below the floor, a draft, a committed piece, a written word. Whatever it shows is a row, and a change of text is a new row. The new row rolls in from below the line, blurred, and comes to rest; the row it replaces, the same element it always was, rolls on up and out, blurring and fading, and is let go. Rows are transitions, so a guess that changes again mid-roll turns smoothly from where it is, and a cell is clipped a little beyond its box, so a row emerges from it and a glow is not cut. What that puts on screen at one position is the model's own sequence at the model's own pace: guesses turning below the floor, one coming into focus as a draft when it clears the floor, the guesses the model drops rolling past, and the committed word rolling in last, or landing where it is when its own draft already had it right. The order and the pace are the recording's; the roll is authored.
...
The prior itself is blank. A guess the model makes at four or more open positions at once says nothing about any of them, so the carve draws it as reserved space, and the reel turns only where the model is guessing something about the position. What is left below the floor is a sparse field of position-specific smears, each floating slowly and turning at the model's own pace. Every change of width in the zone is a slide of at least 520 ms on a soft curve, varied a little by position, and a change that lands mid-slide turns from where the slide is, so the line breathes instead of jumping.
