...
After Tokens asks how an answer from this kind of model should reach a reader, and answers with a system called Settle. An answer has two surfaces and a margin. The page holds released passages as ordinary, still, selectable text. The field is carved into the text after it: an open position reserves blank space, an open position the model already has a confident guess for shows that guess as a draft in a ghost that sharpens with its probability, a committed piece of a word stands as the piece it is, and a word is written where it will stand, dim, the moment every piece of it is in, in the sampler's own order, rolling in from below the line and building in place letter by letter from the middle of the word outward, while the zone shortens from the tail as the model decides the answer's length. Every position is a reel: below the floor the model's guess turns there as a smear blurred past reading, the guesses it drops roll past, and the word it commits rolls in last and stops with a bounce. A line break, committed or confidently guessed, is drawn as a break, so the message's shape arrives before its words. Between the page and the carved zone, the forming text: committed, in-order, word-complete text that has not yet completed a passage, brighter, brightening into the page when it does. When a sentence closes, the page sets: its words press and come to rest where they were written, the ink rises through their letterforms, and a glow cools around them. The margin says what the source is doing, in words, beside a mark whose shape, bloom, breath and hue are a brand's.
...
| motion that carries mass (Lasseter, 1987; Thomas and Johnston, 1981; Chang and Ungar, 1993) | craft arguments and a design paper with no user study: squash and stretch defines an object's rigidity and mass, and solidity and reinforcement make an interface's changes easier to follow | a written word takes shape where it already stands, letter by letter from a blur, and it stops with one bounce; nothing is claimed as measured benefit |
| motion in the periphery (Bartram, Ware and Calvert, 2003) | motion beside a primary task is detected far better than a color change; traveling and zooming icons were rated most distracting, slow linear motion and slow blink least; distraction was self-reported, on notification icons beside a task rather than on text | a guardrail: nothing blinks and nothing travels across the answer; nothing loops but a draft's breath, a change of guess is a short roll in place, a word stops with one bounce, and the motion in the text is a row rolling and a word coming into focus where it stands |
...
2. **The page holds still after it sets.** A passage arrives once, whole, and sets when its sentence closes: its words press and come to rest exactly where they were written while the page's ink rises through them. After that it never changes, moves, blurs or reweights.
...
3. Text on the page never changes, moves or reflows, with one exception: when a sentence closes it sets. Each released word presses down by a twentieth of an em and comes to rest exactly where it was written, in a ripple across the sentence, while the page's ink fills its letterforms from the bottom up with a glint of the accent at the rising edge and a glow of the accent around the passage cools. After the set a word never moves, changes or reweights. The zone after the page reflows as blanks, drafts and pieces become words, each position's width sliding from what it last drew to what it draws now, and a width that shrinks gliding over 2.2 times the drift, so a position gives back space more slowly than it takes it. An available word is drawn in a secondary ink that clears 4.5:1 on both of the brand's grounds.
...
5. Out-of-order text appears only after the page, never inside it. Every position is a reel: whatever it shows is a row, a change of text is a new row, and a guess the model drops rolls up and out as the next rolls in beneath it. An open position reserves a token's width of blank space, 4ch; a guess below the floor turns there as a spin, a smear blurred past reading of the secondary ink drawn only for a piece with letters, which keeps that same width whatever it says, so nothing below the floor is drawn as legible text; a confident guess at one is drawn as a draft, in a ghost that sharpens with its probability; a committed piece of a word is drawn as the piece it is; and a word is written where it will stand, in the secondary ink, only after every piece of it has committed, rolling in last and stopping the way a casino reel stops, with an overshoot and a bounce, building in place letter by letter from the middle of the word outward, or landing where it is with the same bounce when its own draft already had it right. Nothing travels across the answer; a line break, committed or confidently guessed, is drawn as a break; no build precedes a commitment.
...
10. Reduced motion removes the breath, the bloom, the onset, the reel's roll, the bounce, the build, the drafts' lift and breath, the refocus a cell plays when it changes lines, and the set's press, glint and glow, and nothing else. Rows stand where they are, a smear stands where it is at its weight, the page's ink arrives at once, and written words and their letters are drawn sharp at once. What is drawn, and when, is identical.
...
The recordings hold, at every denoising step, the model's provisional argmax for every position it has not committed, with that guess's probability. Those are the drafts, and they are what makes the wait watchable: the answer goes from a rough draft to a polish to a final, in the model's own order, with nothing invented. A guess is drawn once its probability reaches 0.25, the one floor the piece already used, in a ghost of the secondary ink whose opacity and sharpness follow the probability. It keeps being drawn while its text holds, so a guess sitting at the floor does not blink. It is drawn only where a reader can place it, so a guess that continues a word waits for letters before it to attach to, and a guess that the answer ends here is kept in the state and draws no mark, because the cut already says where the answer ends. When the model changes its mind, the reel turns: the guess it drops rolls up and out, blurred, as the new one rolls in beneath it, once per change. A commitment ends the guess at its position, a completed answer holds none, and no guess ever reaches the page. The line the audit demanded now runs in both directions: nothing committed is drawn as a guess, and nothing guessed is drawn as committed.

### The reel

The reserved space is the message's shape, and it is drawn first: every open position holds a token's width, 4ch. Every position of the zone is one element, whatever it happens to show: reserved space, a guess below the floor, a draft, a committed piece, a written word. Whatever it shows is a row, and a change of text is a new row. The new row rolls in from below the line, blurred, and comes to rest; the row it replaces, the same element it always was, rolls on up and out, blurring and fading, and is let go. Rows are transitions, so a guess that changes again mid-roll turns smoothly from where it is, and a cell is clipped a little beyond its box, so a row emerges from it and a glow is not cut. What that puts on screen at one position is the model's own sequence at the model's own pace: guesses turning below the floor, one coming into focus as a draft when it clears the floor, the guesses the model drops rolling past, and the committed word rolling in last, or landing where it is when its own draft already had it right. The order and the pace are the recording's; the roll is authored.

Below the floor the row is a smear. The model's provisional argmax for an open position is recorded at every step, at any probability, and it is drawn as a smear blurred past reading of the secondary ink: the letters are transparent and only their blurred shadow shows, faintest at no confidence and strongest at the floor. Only a piece with letters or digits smears, so the message's shape is still carved from confident guesses alone. The reason it is a smear is in the recordings: below the floor the guess is usually the same word at many positions at once, the corpus prior or a word out of the prompt, "the" or "metaphor" in every open slot. The first build of the recorded mode drew exactly that, legibly, and it was cut for it. What is worth showing is that the reel is turning wherever the model is guessing: the turning is the information, and the word is not, so it is drawn as a smear blurred past reading, and the floor still decides what becomes legible.

The prior itself is blank. A guess that stands at four or more open positions at once says nothing about any of them, so it is drawn as reserved space, and the reel turns only where the model is guessing something about the position. The reducer holds the prior, and holds it steady: a guess becomes the prior at four open positions and stays the prior while it stands at two, so a count wavering across the line does not switch a whole field of smears on and off between steps. What is left below the floor is a sparse field of position-specific smears, turning at the model's own pace. Every change of width in the zone is a slide of at least 520 ms on a soft curve, varied a little by position, and a change that lands mid-slide turns from where the slide is, so the line breathes instead of jumping.

The shape arrives with the reserved space. A line break, once committed as part of a token, is drawn as a break, and so is a line break the model only guesses above the floor, so a list or a set of paragraphs shows its outline while its words are still open. In the recordings the block sampler's list answers commit their breaks with the sentence-ending token before them, and guessed breaks appear as drafts; both are drawn as they arrive and neither is invented. The zone shortens from the tail as the model commits its end, so the length of the answer is carved out in view.

The construction lives in the text itself. When a word is written, every piece of it and its boundaries committed, it stops the way a casino reel stops: it rolls in from 0.9 em below the line over 640 ms, overshoots to 0.075 em above the line, drops back to 0.028 em below it and comes to rest, glowing in the accent as it stops, while its letters build in place, each coming into focus from a blur on its own beat, 460 ms a letter with a 34 ms delay per rank, in two passes, 2.2 px of blur to 1 px, a hold, then sharp, in an order that runs from the middle of the word outward with the two sides alternating. A word whose text was already standing at that position as its own draft or piece lands where it is with the same bounce and no roll, over 520 ms, 0.06 em down, 0.05 em up, 0.02 em down and rest, and the cell's blur clears as the ghost becomes ink. The letters stand in the secondary ink at full opacity on an unchanged ground from the first frame, so a written word clears the contrast floor at every moment and the softness is the blur alone; the roll and the bounce are transform and blur only. Words written at the same instant build at the same instant, in whatever places the sampler committed them, which is what a sampler filling positions in parallel looks like. The build starts the moment the word is written, after its commitment, and no word waits for the one before it, so the answer never reads as a typewriter.

Two arguments hold it together. Traveling motion beside a primary task was rated the most distracting kind and slow motion in place among the least, with blinking the worst (Bartram, Ware and Calvert, 2003, self-reported, measured on notification icons beside a primary task), so nothing loops in the zone except a draft's breath, nothing blinks, and nothing travels across the answer: a change of guess is a short roll in place, and a word stops with one bounce. And blur is the ordinary production signal for something materializing (Emil Kowalski's published guidance, which also holds that ambient looping motion is used only when explicitly asked for), which is what the smear, the reel, the build and the drafts are made of; the one loop left is a draft's breath, it was asked for, it pauses with the recording, and reduced motion removes the roll, the bounce and the build and draws written words sharp at once.

The line the audit demanded holds in both directions here too: a dropped guess rolls away as a ghost in the draft register and never as a word, nothing below the floor is drawn as legible text, and no guess reaches the page.

### The set

A sentence closing is the one moment the page moves, and it moves once. Each released word presses down by a twentieth of an em and comes to rest exactly where it was written, in a ripple across the sentence, 9 ms a word and at most twelve words deep, over 1.9 times the voice's onset, while the page's ink fills its letterforms from the bottom up with a glint of the accent at the rising edge, and a glow of the accent around the whole passage's letterforms cools over 720 ms. A phone that can tick, ticks, for 12 ms. After the set the word never moves, changes or reweights, and it ends exactly where it began. The bloom was drawn first as a flat highlight behind the passage and rejected: a filled rectangle behind a run of words reads as a text selection, so the glow follows the letterforms.

Nothing here is claimed to help a reader. The rows on the reel are the model's own guesses at that position, at every probability, in the model's own order, the breaks are real tokens or real guesses, the build is decoration attached to real commit events that never precedes a commitment, and the set answers a sentence boundary the policy has already released.

### Holding still

The zone reflows for the whole run, and looking at the live page produced one note: the formatting and the lines still jitter and jump forward too sharply. The surface can measure that about itself, so it does. It samples every cell on every animation frame and reports how far each legible cell moved, where a legible cell is a written word, a committed piece or a draft. Most frames were already still; the trouble was in the tail of the distribution, a whole-zone rewrap when a word landed wider than the space its positions had been holding.

Five changes came out of the measurement. An open position reserves 4ch where it used to reserve 2.3ch, because across all 51 usable recordings a content token renders 2.6ch wide on average in the surface's own face, so a word now arrives into space close to what it had; a smear keeps that width whatever it says. A width that shrinks glides over 2.2 times the drift, so a position gives back space more slowly than it takes it and a guess that comes and goes within a few steps barely moves the line. A smear takes no part in the line's width at all: its rows are out of the flow, centered in the position and clipped at its sides, so the reel below the floor turns without moving anything. A cell that inline flow moves to another line comes back into focus where it landed, a 320 ms blur in with no change of color, so a committed word holds its contrast at every frame, and none of it runs under reduced motion. And the page's kerning and ligatures are off: a word in the zone is built from one span a letter and on the page it is one run of text, so without kerning and ligatures the two measure the same and a sentence moving to the page moves nothing, the largest movement of any legible cell at a release being 0.2 px.

What that is worth, on the hook's recording, sampled every animation frame for nine seconds:
...
Typical movement and the worst percentile both fell by about a third. The largest single reflow is where it was: when a word lands early in the zone and the lines after it rewrap, that is still one large movement, and this round did not solve it. The prior moved into the reducer and gained its hysteresis in the same round, for the same reason. These are the surface's numbers about itself, and no reader has been measured.
...
- **The shape first, and no body between the reader and the text.** The earlier cursor sat on the lines, darted to each committed word and jittered on a phone; the companion that replaced it hovered in the open space and answered each word with motes, then with a nod; both put a character between the reader and the text. What remains is the reserved space itself and the message's breaks drawn as soon as they are committed or confidently guessed, so the size and the formatting of the answer are settled in view before its words. The reserved space was drawn for one round as a stream of light with a slow flock along it; the space itself was carrying the argument, and the light was cut the same day. The build's order and the timings are authored; the open positions, the breaks and the commitments are the source's.
- **The reel and the set.** The guesses the model drops became visible first: every position is a reel, and they roll past it, blurred, until the committed word rolls in last. A sentence closing became the one movement the page makes: it sets, with a press, a ripple, the ink rising through the letterforms, and a glow that cools. A flat highlight behind the passage was tried first and read as a text selection. The stream of light under the open positions was rebuilt in the same round and cut in the next one, the same day, at the user's direction: the words and their scrambling carry everything the zone has to say, and the bands of light were decoration laid on top of them. What replaced the light is more of the reel. The model's guess for an open position is now recorded at every probability, so the reel turns below the floor as well as above it, as a smear blurred past reading, and the committed word stops on it the way a casino reel stops, with an overshoot and a bounce. The prior came out the same evening: a guess that stands at four or more open positions at once is drawn as blank, and holds while it stands at two, and the zone's slides were lengthened and made to turn from the width on screen, after a slide that restarted from its origin had been making the words jump. The round after that measured the line's stability per cell and per frame and rebuilt the widths against the measurement. Two ways of quieting the reflow were tried on the same measurement and rejected by it: rounding every standalone guess up to a whole number of token widths, and holding each uncommitted position at the widest space it had reached. Both widened the zone, and both made the movement worse.
