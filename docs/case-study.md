...
| motion that carries mass (Lasseter, 1987; Thomas and Johnston, 1981; Chang and Ungar, 1993) | craft arguments and a design paper with no user study: squash and stretch defines an object's rigidity and mass, and solidity and reinforcement make an interface's changes easier to follow | a written word takes shape where it already stands, letter by letter from a blur, and the stream's flock hovers at one speed inside its own position; nothing is claimed as measured benefit |
| motion in the periphery (Bartram, Ware and Calvert, 2003) | motion beside a primary task is detected far better than a color change; traveling and zooming icons were rated most distracting, slow linear motion and slow blink least; distraction was self-reported, on notification icons beside a task rather than on text | a guardrail: nothing blinks and nothing travels across the answer; the flock hovers slowly inside its own position and never leaves it, and the motion in the text is a row rolling and a word coming into focus where it stands |
...
5. Out-of-order text appears only after the page, never inside it. Every position is a reel: whatever it shows is a row, a change of text is a new row, and a guess the model drops rolls up and out as the next rolls in beneath it. An open position is reserved blank space; a confident guess at one is drawn as a draft, in a ghost that sharpens with its probability; a committed piece of a word is drawn as the piece it is; and a word is written where it will stand, in the secondary ink, only after every piece of it has committed, rolling in last and building in place letter by letter from the middle of the word outward, or landing where it is when its own draft already had it right. Under every position runs the stream, lit on an open position, just over half as strong under a draft, and dissolving under a word as it lands; its shimmer runs on scrambled phases, so no wave travels along the line, and the flock hovers inside its own position. Nothing travels across the answer; a line break, committed or confidently guessed, is drawn as a break; no build precedes a commitment.
...
10. Reduced motion removes the breath, the bloom, the onset, the reel's roll, the build, the drafts' lift and breath, the stream's shimmer and its flock, and the set's press, glint and glow, and nothing else. Rows stand where they are, the band stands still, the page's ink arrives at once, and written words and their letters are drawn sharp at once. What is drawn, and when, is identical.
...
The reserved space is the message's shape, and it is drawn first. Every open position is about a token wide, and every position of the zone carries the stream: a band of the brand's light at the height of the lowercase letters, the cell's own light, lit while the position is open, just over half as strong under a draft, since the position is still open, and dissolving to nothing under a piece or a written word, so the light dissolves under a word as the word lands and never retreats from the words. Its shimmer rises and falls slowly on a phase that is scrambled from one position to the next, so neighbors are never in step and no wave runs along the line. Over about every third open position hovers the flock: a glowing dot on a small closed path inside its own position, a 5.2 second loop at constant speed, which it never leaves. It stands where words may be but are not decided yet, so the bubble is never blank and never noisy, and a reader can see how big the answer is before a word of it is final. The stream shortens from the tail as the model commits its end.
...
The construction lives in the text itself. When a word is written, every piece of it and its boundaries committed, it rolls in and builds in place: each letter comes into focus from a blur on its own beat, 460 ms a letter with a 34 ms delay per rank, in two passes, 2.2 px of blur to 1 px, a hold, then sharp, in an order that runs from the middle of the word outward with the two sides alternating, while the word as a whole rolls up from 0.85 em below the line over 560 ms, overshoots by a hair and settles, glowing in the accent. A word whose text was already standing at that position as its own draft or piece lands where it is: the reel stops, with the same rise, overshoot and glow and no roll, and the cell's blur clears as the ghost becomes ink. The letters stand in the secondary ink at full opacity on an unchanged ground from the first frame, so a written word clears the contrast floor at every moment and the softness is the blur alone. Words written at the same instant build at the same instant, in whatever places the sampler committed them, which is what a sampler filling positions in parallel looks like. The build starts the moment the word is written, after its commitment, and no word waits for the one before it, so the answer never reads as a typewriter.

Two arguments hold it together. Traveling motion beside a primary task was rated the most distracting kind and slow motion in place among the least, with blinking the worst (Bartram, Ware and Calvert, 2003, self-reported, measured on notification icons beside a primary task), so nothing travels across the answer, the flock hovers at constant speed inside its own position and never leaves it, the shimmer is slow and runs on scrambled phases so no wave crosses the line, and nothing blinks. And blur is the ordinary production signal for something materializing (Emil Kowalski's published guidance, which also holds that ambient looping motion is used only when explicitly asked for), which is what the reel, the build and the drafts are made of; the loop here was asked for, it pauses with the recording, and reduced motion stills the band, removes the flock and the roll, and draws written words sharp at once.

### The reel

Every position of the zone is one element, whatever it happens to show: reserved space, a draft, a committed piece, a written word. Whatever it shows is a row, and a change of text is a new row. The new row rolls in from below the line, blurred, and comes to rest; the row it replaces, the same element it always was, rolls on up and out, blurring and fading, and is let go. Rows are transitions, so a guess that changes again mid-roll turns smoothly from where it is, and a cell is clipped a little beyond its box, so a row emerges from it and a glow is not cut. What that puts on screen at one position is the model's own sequence: a guess rolls up out of the stream, the guesses the model drops roll past, and the committed word rolls in last, or lands where it is when its own draft already had it right. The order and the pace are the recording's; the roll is authored. The line the audit demanded holds in both directions here too: a dropped guess rolls away as a ghost in the draft register, never as a word, and no guess reaches the page.
...
Nothing here is claimed to help a reader. The stream stands on real open positions, the breaks are real tokens or real guesses, the rows on the reel are the model's own guesses at that position in the model's own order, the build is decoration attached to real commit events that never precedes a commitment, and the set answers a sentence boundary the policy has already released.
...
- **The reel and the set.** The stream, as first built, ran its shimmer on a phase that stepped along the line and sent its flock across each position, so the light read as a wave traveling forward with the words, and the band went out at the instant a word arrived, so the light appeared to retreat from the words instead of the words landing in it. The band now belongs to every cell and dissolves under a word as it lands, the shimmer's phases are scrambled, and the flock hovers inside its own position. The guesses the model drops became visible at the same time: every position is a reel, and they roll past it, blurred, until the committed word rolls in last. And a sentence closing became the one movement the page makes: it sets, with a press, a ripple, the ink rising through the letterforms, and a glow that cools. A flat highlight behind the passage was tried first and read as a text selection.
