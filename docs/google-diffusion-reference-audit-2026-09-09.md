...
The useful design direction is a field that can absorb distributed changes without asking the reader to track every unstable word. Keep the paragraph margin and typographic rhythm legible; do not turn arbitrary decorative cells into claims about exact tokens, line breaks, confidence or percentage complete. The current waiting field follows an independent ambient score, so draft revisions do not continually reshape the surface.

The earlier v6 experiment used current content to estimate space, with different contracts for irreversible positions and revisable snapshots. V7 deliberately removes that dependency from waiting geometry. The integration must still distinguish provisional text from eligible passages and real finality. The full final page can be measured only once the final answer arrives; an already policy-eligible passage can be measured earlier. A fixed waiting allocation may need a final-size adjustment before the material handover.

The three-frame inspector is a qualitative source reference. Any authored revision example next to it must disclose its invented events and clock, and must not be presented as a replay of Google’s API or measured timings. This archive supplies no throughput estimate, perceived-wait result, comprehension result, comparative superiority claim or evidence of first invention. See the [current motion rationale and implementation handoff](ambient-skeleton-v7-handoff-2026-09-12.md) for the project’s own contracts and testable claims.
