...
The useful design direction is a field that can absorb distributed changes without asking the reader to track every unstable word. Keep the paragraph margin and typographic rhythm legible; do not turn arbitrary decorative cells into claims about exact tokens, line breaks, confidence or percentage complete. In the current v8 treatment, individual bar widths follow independently seeded motion. A separate, coarse estimate of currently received content can reserve overall height, without copying a candidate’s line layout.

V6 estimated both row occupancy and space from current content; v7 used a fixed five-row allocation. V8 restores only the coarser height estimate, with different contracts for irreversible positions and revisable snapshots. It does not restore draft-driven row widths. The integration must still distinguish provisional text from eligible passages and real finality. The full final page can be measured only once the final answer arrives; an already policy-eligible passage can be measured earlier. A coarse allocation may still need a final-size adjustment before the material handover.

The three-frame inspector is a qualitative source reference. Any authored revision example next to it must disclose its invented events and clock, and must not be presented as a replay of Google’s API or measured timings. This archive supplies no throughput estimate, perceived-wait result, comprehension result, comparative superiority claim or evidence of first invention. See the [current motion rationale and implementation handoff](growing-skeleton-v8-handoff-2026-09-12.md) for the project’s own contracts and testable claims.
