...
I show the authored introduction’s answer-reveal timeline as a small, translucent percentage pill inside the reply’s bottom-right corner. It pauses with the introduction and reaches 100% only after the final sentence’s visual handover. A 160 ms hold and 420 ms opacity/transform exit let completion register before docking. Tabular numbers keep its width steady; reduced motion and Skip hide it. This is presentation progress, not a measurement of model confidence or inference completion.

The same pill sits at the right of the status row in every demonstration: the shared comparison, policy stages, motion study, revision exercise, and desktop/search/phone previews. Every instance uses its existing replay clock and pauses with that clock. The shared renderer holds it below 100% until source completion and the visual handover; stopped or failed sources never receive a success fade. When a source has no known duration, the reusable component leaves the percentage indeterminate. I do not derive it from token positions, provisional words, or an assumed amount of remaining live inference. Motion-off and reduced-motion modes remove the exit movement.
