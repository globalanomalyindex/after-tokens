...
- Words lock in local clusters from a few anchors on a growth process fitted to the recorded adjacency (51%) and anchor rate (finding 02; change blindness).
- The cadence is linear at 45 to 80 ms per word after a 320 ms pre-roll, because the recorded cadence is linear within 0.119 of a straight line (finding 01; Doherty threshold). Phi decay is retired to the comparison stimulus.
