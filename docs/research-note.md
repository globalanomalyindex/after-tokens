...
`lib/settle/carve.ts` derives the carved zone: for every position after the word-safe prefix, an open slot, the source's current guess where it holds one above the floor (section 9.4), a committed piece of a word whose other pieces or boundaries are out, a complete word (every piece committed, with a clean start and a boundary after it), or a collapsed run of end tokens. Complete words are drawn where they will stand, dim, in the sampler's own order; a piece is drawn as the piece it is, since its letters are committed; an open slot without a guess is never letters. `lib/settle/phase.ts` reads a phase off the same state for the margin: the positions the answer can still occupy are the ones up to the lowest committed end, else the request's bound, and the committed share plus half the drafted share puts the run in sketching (under 0.15), drafting (under 0.5), polishing (under 0.85) or closing. The thresholds are authored and the line promises nothing about what comes next. `lib/settle/field.ts` derives the strip, one cell per position (released, forming, held, committed, end, open, beyond), the field's compact form, which is state and never text. Beside the drafts, the same script writes `spins` into the compact trajectories: one list per step of the provisional argmax at an open position at any probability, recorded whenever that guess changes, with the first step recording every position and a commitment ending its position's spin (`data/traces/README.md`). The carve draws one only where no draft is shown and only for a piece with letters or digits, as a smear blurred past reading of the secondary ink whose letters are transparent and whose blurred shadow alone shows, weighted by the probability's share of the floor. So the reel turns wherever the model is guessing, and nothing below the floor is drawn as legible text. A guess that stands at four or more open positions at once is the prior for an unknown position and is drawn as blank space, and it stays the prior while it stands at two or more, so a count that wavers across the line does not switch a field of smears on and off between steps; the reducer computes the prior after every spin and every commitment and keeps it in the state (`PRIOR_ENTER` and `PRIOR_STAY` in `lib/settle/reader.ts`), and the carve reads it from there, so the reel turns only where the guess is specific to the position. An open position reserves 4ch of blank space, a token's width: a content token renders 2.6ch wide on average across all 51 usable recordings in the surface's own face, so a word arrives into space close to the space its positions held. Every change of width in the zone slides over at least 520 ms on a soft curve, a width that shrinks glides over 2.2 times as long, and a change that lands mid-slide turns from the width on screen. A line break committed as part of a token, or guessed above the floor, is drawn as a break, so the message's shape is legible before its words. Every position is also a reel: whatever it draws is a row, and a change of text is a new row that rolls in from below the line as the row it replaces rolls up and out, blurred, so the guesses the model drops at that position pass in the model's own order and the committed word rolls in last and stops the way a casino reel stops, rolling up from below the line, overshooting and dropping back to rest, or landing where it is with the same bounce when its own draft already stood there. A written word builds in place, each letter coming into focus from a blur on its own beat from the middle of the word outward, so several words written at once build at once. Nothing travels across the answer. On the page, a sentence closing sets: each released word presses down by a twentieth of an em and comes back to rest where it was written, in a ripple across the sentence, while the page's ink fills its letterforms from the bottom up with a glint of the accent at the rising edge and a glow around the passage cools. After the set nothing on the page moves again.
...
- Motion has principles where it does not have measurements. Squash and stretch defines an object's rigidity and mass by distorting its shape during an action, and anticipation and follow-through place that action in time; this is a craft argument rather than a study (Lasseter, 1987, applying the principles the Disney studio developed in the 1930s, which Thomas and Johnston codified in 1981 without inventing them). Applying solidity, exaggeration and reinforcement to interface objects, so that changes are easier to follow, is a design and implementation paper with no user study and cannot be cited as evidence of measured benefit (Chang and Ungar, 1993). Where interface motion has been measured: animated transitions between statistical graphics significantly beat abrupt changes for object tracking and estimation, with simple staging helping modestly and being preferred while heavy staging hurt (Heer and Robertson, 2007); and in peripheral vision, motion is detected far better than a color change, with traveling icons rated the most distracting, then zoom, and slow linear motion and slow blink the least, so the authors recommend slow linear motion as the compromise (Bartram, Ware and Calvert, 2003, with distraction self-reported and the task a notification study with small icons beside a primary task, so it bears on peripheral motion rather than on animated text). Consequence: nothing travels across the answer; nothing loops in the zone but a draft's breath, no element blinks, every transition in the zone is a slide, a roll or a blur, and the reel's rows move by transform and blur alone; the one overshoot is the bounce a word stops on, and the one movement on the page is the set when a sentence closes, over within the set itself. The surface's own movement was measured before and after the round of 8 September that reworked the zone's widths, sampling every animation frame of one recording: the mean movement per legible cell per frame was 3.69 px before and 2.38 px after. That is the surface measuring itself, and no reader was measured.
