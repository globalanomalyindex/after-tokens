...
Date: 7 September 2026. Base: ab95e6a. Status: design locked, implementing. Amended 8 September 2026, in place, for the draft register, the piece, the build, the stream, and the phase in the margin.
...
5. Out-of-order text appears only after the page, never inside it. An open position is reserved blank space about a token wide. An open position the source holds a confident guess for draws that guess as a draft: the source's own provisional argmax at that instant, in a ghost of the secondary ink whose opacity and sharpness follow its probability, and it reconsiders visibly when the source changes its mind. A committed piece of a word that is not complete is drawn as the piece it is, because its letters are facts. A word is written where it will stand, in the secondary ink, only after every piece of it and its boundaries have committed, and it builds in place: each letter comes into focus from a blur on its own beat, 460 ms a letter with a 34 ms delay per rank, in two passes (2.2 px of blur to 1 px, a hold, then sharp), in an order that runs from the middle of the word outward with the two sides alternating, while the word as a whole rises slightly and glows in the accent for 520 ms and its width slides from the width its positions actually held to its own. The letters stand in the secondary ink at full opacity on an unchanged ground from the first frame, so a written word clears the contrast floor at every moment and the softness is the blur alone. Words written at the same instant build at the same instant, in whatever places the sampler committed them, which is what a sampler filling positions in parallel looks like. Across the open positions runs the stream: a faint band of the accent's light at the height of the lowercase letters, one per open position, touching so a row of them reads as one stream, whose brightness slowly rises and falls out of step from one position to the next (a 4.4 s alternation, offset 430 ms per position); along it drifts the flock, a glowing dot of 0.22 em on about every third open position, each crossing its position in 3 s at constant speed on its own phase, fading in and out: where words may be but are not decided yet. A line break, once committed or confidently guessed, is drawn as a break, so the message's shape, its length, its paragraphs, its list, arrives before its words; the stream shortens from the tail as the length is decided. Nothing travels across the answer.
...
10. Reduced motion removes the mark's breath, the cell bloom, the onset, the word's build, the drafts' lift, the draft's reconsidering and its breath, and the stream's shimmer and its flock; the stream stands still and a written word and its letters are drawn sharp at once. It changes nothing else, and it changes nothing about what is drawn or when.
...
- The build: when every piece of a word and its boundaries are in, the word builds in place. Each letter comes into focus from a blur on its own beat, 460 ms a letter with a 34 ms delay per rank, in two passes (2.2 px of blur to 1 px, a hold, then sharp), in an order that runs from the middle of the word outward with the two sides alternating (`buildOrder` in the component), so no letter order is left to right; the word as a whole rises slightly and glows in the accent for 520 ms. The letters are in the secondary ink at full opacity on an unchanged ground from the first frame, so a written word clears the contrast floor at every moment and the softness is the blur alone. Words written at the same instant build at the same instant, in whatever places the sampler committed them, which is what a sampler filling positions in parallel looks like. The build starts the moment the word is written, after its commitment, and no word waits for the one before it. Purpose: the one moment in the zone worth marking is the moment a guess becomes a fact, and the word takes shape where it already stands.
...
- The draft's breath: after the 380 ms reconsidering, a draft's letters alternate slowly between full presence and 0.78 over 4.2 s, offset by 530 ms per position. Purpose: the unresolved part of the answer is never still and never blinks. None under reduced motion.
...
- The stream and the flock: Across the open positions runs the stream: a faint band of the accent's light at the height of the lowercase letters, one per open position, touching so a row of them reads as one stream, whose brightness slowly rises and falls out of step from one position to the next (a 4.4 s alternation, offset 430 ms per position); along it drifts the flock, a glowing dot of 0.22 em on about every third open position, each crossing its position in 3 s at constant speed on its own phase, fading in and out: where words may be but are not decided yet. A line break, once committed or confidently guessed, is drawn as a break, so the message's shape, its length, its paragraphs, its list, arrives before its words; the stream shortens from the tail as the length is decided. Nothing travels across the answer. Purpose: the undecided part of the answer is alive without being noisy, and its size and shape are legible before a word of it is final; slow motion in place is the least distracting kind and blinking the worst (Bartram, Ware and Calvert, 2003, measured on notification icons beside a primary task), so the flock drifts at constant speed, the shimmer is slow, and nothing blinks or hurries. Both pause with the recording (`data-paused`).
- The ring and the close: a sentence closing expands one ring over 640 ms, and a phone that can tick, ticks; when the source is done the body swells, softens and dissolves over 1.1 s.
- The body's loop runs only while its surface is on screen and the recording is playing. Pausing the recording holds the body, the wisps and the breath still. Only the body's transform is written by the surface, once per frame; the shape, the wisps, the ring and the breath are the stylesheet's.
- Nothing else moves. No entrance stagger on the page, no halo, no weight change, no exhale, and no body of any kind between the reader and the text. Under reduced motion the stream stands still with no flock, the drafts neither breathe nor lift, and written words and their letters are drawn sharp at once.

### 5.3 Accessibility

The page is a `region` labeled as the answer; passages are plain spans. The zone after the page is `aria-hidden`: it is the source's state drawn where the words will stand, and the status words carry its meaning. The status words live in a polite, atomic live region and change only on state changes, and never on every commit. The stream and the flock are pseudo-elements of the zone's open positions, hidden with it: decoration on real open positions that carries nothing the status words do not say. Their motion pauses with the recording and is removed under reduced motion. Controls are native buttons and selects. Reduced motion is honored in CSS and in the choreography. Contrast: page text and every committed letter in the zone (a piece, a written word at every moment of its build) at least 4.5:1 on every brand, through the secondary ink's floor. A draft is exempt as incidental, provisional text: it is the source's guess drawn as a ghost on purpose, the answer never depends on it, and a reader who cannot see it loses nothing the contract promises. The axe check in `tests/e2e/a11y.spec.ts` excludes drafts and the register legend's decorative samples on that basis, and holds everything else to the floor.
...
10. The stream runs only across open positions and nothing travels across the answer; a break is drawn only for a committed or confidently guessed line break.
