...
Date: 7 September 2026. Base: ab95e6a. Status: design locked, implementing. Amended 8 September 2026, in place, for the draft register, the piece, the build, the spins, the phase in the margin, the reel, and the set.
...
10. Reduced motion removes the mark's breath, the cell bloom, the onset, the reel's roll, the word's bounce, the word's build, the drafts' lift, the draft's reconsidering and its breath, and the set's press, glint and glow; rows stand where they are, a smear stands where it is at its weight, the page's ink arrives at once when a sentence closes, and a written word and its letters are drawn sharp at once. It changes nothing else, and it changes nothing about what is drawn or when.
...
- The width slide: a position's width slides from the width it last drew (blank, smear, draft, or pieces) to the width it draws now over the drift: at least 520 ms, or the onset where that is longer, varied by position from 0.9 to 1.1 times so the line breathes rather than moving in step, on the strong ease-out. A change that lands mid-slide turns from the width on screen (the slide once restarted from the width it began at, which made the words jump). The surface remembers each position's last drawn width. Purpose: the line moves instead of jumping.
...
12. A guess made at four or more open positions at once is drawn as blank, whatever its spacing or case, and a committed position no longer counts toward it (`tests/settle/carve.test.ts`).
