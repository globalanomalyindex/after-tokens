...
Date: 7 September 2026. Base: ab95e6a. Status: design locked, implementing. Amended 8 September 2026, in place, for the draft register, the piece, the snap, the companion, and the phase in the margin.
...
5. Out-of-order text appears only after the page, never inside it. An open position is reserved blank space about a token wide. An open position the source holds a confident guess for draws that guess as a draft: the source's own provisional argmax at that instant, in a ghost of the secondary ink whose opacity and sharpness follow its probability, and it reconsiders visibly when the source changes its mind. A committed piece of a word that is not complete is drawn as the piece it is, because its letters are facts. A word is written where it will stand, in the secondary ink, only after every piece of it and its boundaries have committed, and it snaps in where it stands: 340 ms of blur to sharp, a slight rise, and a glow of the accent around the letters that fades as the word dries in place, while its width slides from the width its positions actually held to its own. The companion, a small soft body in the brand's color, lives in the answer's open space and never sits on a line of text: it hovers over the middle of what is still open, in the gap under the last line written, and drifts after it on a soft spring as the answer fills. It never goes to the words. When a word is written the companion sends motes to it, and the word snaps as the first one lands; several words written at once get several beams at once, which is what a sampler filling positions in parallel looks like. It rings once when a sentence closes and dissolves when the source is done. Every mote answers a commitment the source has already made. Nothing committed is drawn as a guess, nothing guessed is drawn as committed, and no guess ever reaches the page.
...
10. Reduced motion removes the mark's breath, the cell bloom, the onset, the word's snap, the drafts' lift, the draft's reconsidering and its breath, and the companion's drift, wisps, motes, ring and dissolve; the companion is placed at its home and a written word is drawn sharp at once. It changes nothing else, and it changes nothing about what is drawn or when.
...
| `concept` | the wait as a product moment | what the surface opens and none of it measured: the companion in the answer's open space, a draft a reader can watch become the answer, a tick under the thumb, a slot for a tip or a card, and time on app with the labor illusion as its guardrail. |
...
components/settle/  settle-answer.tsx   the product component: page, carved zone, companion, field, margin
                    use-companion.ts    the companion's home spring, wander, stretch and motes
...
- The snap: when every piece of a word and its boundaries are in, the word settles in place over 340 ms, from blurred to sharp, with a slight rise and a glow of the accent around the letters, the mote's touch, that fades as the word dries in place. Purpose: the one moment in the zone worth marking is the moment a guess becomes a fact, and it is marked where the word already stands.
...
- The neighbors' lift: when a word snaps, the drafts within three positions on either side lift for 640 ms, brighter and sharper with a slight rise, and then return to their own probability. Purpose: a commitment really does lift its neighbors' confidence (section 4.7), and this draws that where it happens. None under reduced motion.
- The companion: a small soft body in the brand's color that lives in the answer's open space and never sits on a line of text. Its home is measured every 160 ms: horizontally at the centroid of what is still open in the zone (blank slots and drafts), vertically in the gap 0.85 em under the last line written in the zone or on the page; over the first open slot while nothing is written; at the end of the page once the source is done. It reaches home on a soft spring (stiffness 13, damping ratio 1, the measured home itself followed at 0.35 per measurement so the body tracks the trend of it and never every step, about a second, no overshoot), so as the open part of the answer moves the body drifts after it. While it waits it wanders on two slow sinusoids (about 4.5 px, periods between 3 and 6 seconds) and breathes on the brand's breath period; it stretches along its own velocity when it moves, up to 1.32 times, volume preserving, and rounds up as it stops. Purpose: slow motion in view is the least distracting kind and blinking the worst (Bartram, Ware and Calvert, 2003, measured on notification icons beside a primary task), and squash, stretch and follow-through are how a small body reads as having mass and intent (Lasseter, SIGGRAPH 1987, on the Disney studio's principles of the 1930s; Chang and Ungar, 1993, on solidity in interface objects). Its shape is a core of 1.25 em whose outline changes slowly over 9 s, with a soft radial edge and a glow, and two wisps turning around it on tilted paths of 11 s and 8.2 s: a small cloud in the brand's color, bone on the after-tokens stage and cobalt on the concept phone.
- The motes: when a word is written, the body sends three motes to it, 45 ms apart, along a shallow arc that alternates sides, each flying 260 to 440 ms by distance, 0.3 em across and brief. It leans 7 px toward the word for 420 ms and squashes for 220 ms as it fires. The word stands soft (a 1.4 px blur and a slight sink) until the first mote lands, then snaps; its color never changes, so it clears the contrast floor at every frame. Several words written at once get several beams at once, which is what a sampler filling positions in parallel looks like. Where motes cannot be sent (the surface is off screen, reduced motion, or the pool of 36 is exhausted) the word snaps at once. Purpose: the body can answer a commitment anywhere in the answer without leaving the open space or landing on a line of text, and no beam ever precedes a commitment.
...
- The body's loop runs only while its surface is on screen and the recording is playing. Pausing the recording holds the body, the wisps and the breath still. Only the body's transform and the motes' are written by the surface, once per frame; the shape, the wisps, the ring and the breath are the stylesheet's.
- Nothing else moves. No entrance stagger on the page, no halo, no weight change, no exhale. Under reduced motion the companion is placed at its home with no wander, no wisps, no motes, no ring and no dissolve; the drafts neither breathe nor lift, and words arrive sharp, without the snap.

### 5.3 Accessibility

The page is a `region` labeled as the answer; passages are plain spans. The zone after the page is `aria-hidden`: it is the source's state drawn where the words will stand, and the status words carry its meaning. The status words live in a polite, atomic live region and change only on state changes, and never on every commit. The companion is `aria-hidden`: it is decoration attached to real commit events, and it carries nothing the status words do not say. Its ambient motion pauses with the recording and is removed under reduced motion. Controls are native buttons and selects. Reduced motion is honored in CSS and in the choreography. Contrast: page text and every committed letter in the zone (a piece, a written word at every frame of its snap) at least 4.5:1 on every brand, through the secondary ink's floor. A draft is exempt as incidental, provisional text: it is the source's guess drawn as a ghost on purpose, the answer never depends on it, and a reader who cannot see it loses nothing the contract promises. The axe check in `tests/e2e/a11y.spec.ts` excludes drafts and the register legend's decorative samples on that basis, and holds everything else to the floor.
...
9. A written word snaps only after its commitment: as the first mote lands, or at once where motes cannot be sent.
10. The companion never sits on a line of text. It hovers in the gap under the last line written, over the middle of what is still open.
