...
Date: 7 September 2026. Base: ab95e6a. Status: design locked, implementing. Amended 8 September 2026, in place, for the draft register, the piece, the build, the stream, the phase in the margin, the reel, and the set.
...
5. Out-of-order text appears only after the page, never inside it. An open position is reserved blank space about a token wide. Every position of the zone is a reel: whatever it shows is a row, a change of text is a new row, and the new row rolls in from below the line, blurred, as the row it replaces rolls up and out, so at one position the reader sees the source's own sequence, a guess rolling up out of the stream, the guesses the source drops rolling past, and the committed word rolling in last. An open position the source holds a confident guess for draws that guess as a draft: the source's own provisional argmax at that instant, in a ghost of the secondary ink whose opacity and sharpness follow its probability, and when the source changes its mind the reel turns, so the guess it drops rolls away above the line as the new one rolls in beneath it. A committed piece of a word that is not complete is drawn as the piece it is, because its letters are facts. A word is written where it will stand, in the secondary ink, only after every piece of it and its boundaries have committed; it rolls in last of all, overshoots by a hair and settles, glowing in the accent, or lands where it is when its own draft or piece was already standing there, and it builds in place: each letter comes into focus from a blur on its own beat, 460 ms a letter with a 34 ms delay per rank, in two passes (2.2 px of blur to 1 px, a hold, then sharp), in an order that runs from the middle of the word outward with the two sides alternating, while its width slides from the width its positions actually held to its own. The letters stand in the secondary ink at full opacity on an unchanged ground from the first frame, so a written word clears the contrast floor at every moment and the softness is the blur alone. Words written at the same instant build at the same instant, in whatever places the sampler committed them, which is what a sampler filling positions in parallel looks like. Under every position of the zone runs the stream: a band of the accent's light at the height of the lowercase letters, carried by the cell itself, lit on an open position, just over half as strong under a draft, since the position is still open, and transparent under a piece or a written word, so the light dissolves under a word as it lands and never retreats from the words; its shimmer rises and falls on a phase that is scrambled from one position to the next, so neighbors are never in step and no wave travels along the line. Over about every third open position hovers the flock, a glowing dot of 0.22 em on a small closed path inside its own position, which it never leaves: where words may be but are not decided yet. A line break, once committed or confidently guessed, is drawn as a break, so the message's shape, its length, its paragraphs, its list, arrives before its words; the stream shortens from the tail as the length is decided. Nothing travels across the answer.
...
10. Reduced motion removes the mark's breath, the cell bloom, the onset, the reel's roll, the word's build, the drafts' lift, the draft's reconsidering and its breath, the stream's shimmer and its flock, and the set's press, glint and glow; rows stand where they are, the band stands still under open positions and drafts, the page's ink arrives at once when a sentence closes, and a written word and its letters are drawn sharp at once. It changes nothing else, and it changes nothing about what is drawn or when.
...
Authored here: the display floor, the hysteresis, the attach rule, the ghost's ramp, the reconsidering, the breath, and the lift's reach and duration. Recorded: the drafts, their probabilities, their changes, and the neighbor lift the drafts' lift draws.
...
- The reel: every position of the zone is one element, keyed by position, whatever it shows: reserved space, a draft, a committed piece, or a written word. Whatever it shows is a row, and a change of text is a new row. The new row rolls in from 0.9 em below the line, blurred by 3 px, and comes to rest over 460 ms on the exponential ease-out; the row it replaces, the same element it always was, rolls on up and out to 0.95 em above with 2.5 px of blur, fading over 240 ms, and is let go after 520 ms. Rows are transitions, so a guess that changes again mid-roll turns smoothly from where it is. A cell is clipped 0.14 em beyond its box vertically and 0.3 em sideways, so a row emerges from its cell and a glow is not cut. Purpose: at one position the reader sees the source's own sequence, a guess rolling up out of the stream, the guesses the source drops rolling past, and the committed word rolling in last; the order and the pace are the recording's, and the reel adds the roll. None under reduced motion, where rows stand where they are.
- The build: when every piece of a word and its boundaries are in, the word rolls in and builds in place. It rolls in from 0.85 em below the line over 560 ms, overshoots by a hair (0.035 em) and settles, glowing in the accent; a word whose text was already standing at its position as its own draft or piece lands where it is, the reel stopping with the same rise, overshoot and glow and no roll, over 520 ms, while the cell's blur clears over 460 ms as the ghost becomes ink. Each letter comes into focus from a blur on its own beat, 460 ms a letter with a 34 ms delay per rank, in two passes (2.2 px of blur to 1 px, a hold, then sharp), in an order that runs from the middle of the word outward with the two sides alternating (`buildOrder` in the component), so no letter order is left to right. The letters are in the secondary ink at full opacity on an unchanged ground from the first frame, so a written word clears the contrast floor at every moment and the softness is the blur alone. Words written at the same instant build at the same instant, in whatever places the sampler committed them, which is what a sampler filling positions in parallel looks like. The build starts the moment the word is written, after its commitment, and no word waits for the one before it. Purpose: the one moment in the zone worth marking is the moment a guess becomes a fact, and the word takes shape where it already stands.
...
- The stream and the flock: the band of the accent's light at the height of the lowercase letters is carried by every cell of the zone. Its light is its background color: the accent at 30 percent on an open position, and on one where the source only guesses the end; 17 percent under a draft, since the position is still open; and transparent under a piece or a written word, the color transitioning over 620 ms, so the light dissolves under a word as it lands and never retreats from the words. Its shimmer is its opacity, rising and falling between 1.3 and 1.75 times the voice's open alpha over 4.4 s alternating, on a phase per position that is scrambled (the phase index is (position × 7) mod 11, times 430 ms; 7 is coprime with 11), so neighbors are never in step and no wave travels along the line. The flock is a glowing dot of 0.22 em on about every third open position, hovering on a small closed path inside its own position, a 5.2 s loop at constant speed, its phases scrambled the same way and its opacity between 0.55 and 0.9; it never leaves its position. A line break, once committed or confidently guessed, is drawn as a break, so the message's shape, its length, its paragraphs, its list, arrives before its words; the stream shortens from the tail as the length is decided. Nothing travels across the answer. Purpose: the undecided part of the answer is alive without being noisy, and its size and shape are legible before a word of it is final; slow motion in place is the least distracting kind and blinking the worst (Bartram, Ware and Calvert, 2003, measured on notification icons beside a primary task), so the flock hovers at constant speed inside its own position, the shimmer is slow, and nothing blinks or hurries. Both pause with the recording (`data-paused`).
- Nothing else moves. No entrance stagger on the page, no halo, no weight change, no exhale, and no body of any kind between the reader and the text. Under reduced motion the reel's rows stand where they are, the band stands still under open positions and drafts at 1.6 times the open alpha with no shimmer and no flock, the drafts neither breathe nor lift, written words and their letters are drawn sharp at once, and a sentence sets with the page's ink at once, with no press, no glint and no glow.

### 5.3 Accessibility

The page is a `region` labeled as the answer; passages are plain spans. The zone after the page is `aria-hidden`: it is the source's state drawn where the words will stand, and the status words carry its meaning. The status words live in a polite, atomic live region and change only on state changes, and never on every commit. The stream's band is the light of every cell of the zone and the flock a pseudo-element of its open positions, hidden with the zone: decoration on real positions that carries nothing the status words do not say. Their motion pauses with the recording and is removed under reduced motion. Controls are native buttons and selects. Reduced motion is honored in CSS and in the choreography. Contrast: page text and every committed letter in the zone (a piece, a written word at every moment of its build) at least 4.5:1 on every brand, through the secondary ink's floor. A draft is exempt as incidental, provisional text: it is the source's guess drawn as a ghost on purpose, the answer never depends on it, and a reader who cannot see it loses nothing the contract promises. A row leaving the reel is exempt on the same basis: it is the ghost of a guess on its way out. The axe check in `tests/e2e/a11y.spec.ts` excludes drafts, dropped rows (`.settle-zone .settle-cz-text[data-past]`) and the register legend's decorative samples on that basis, and holds everything else to the floor. A roll is transform and blur alone, so committed letters stay in the secondary ink at full opacity from its first frame; and the set keeps the page's text at full contrast, since the press is a transform and the ink only darkens. `tests/e2e/reduced-motion-full.spec.ts` covers the passage and the cells' band as well as the zone.
...
10. The stream's light stands on the open positions and dissolves under a piece or a written word, and nothing travels across the answer; a break is drawn only for a committed or confidently guessed line break.
