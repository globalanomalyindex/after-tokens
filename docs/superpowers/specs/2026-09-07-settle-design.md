...
**The page** is where the reader reads. It holds released passages as ordinary, selectable text in the product's own face. A passage arrives once, whole, and it sets when its sentence closes: its words press and come to rest exactly where they were written while the page's ink rises through their letterforms, over 1.9 times the onset, and none of it under reduced motion (section 5.2). After the set, text on the page never changes, never moves, and never animates.

**The field** is where the reader watches, and it is carved into the text (8 September). Every position after the word-safe prefix is drawn as what the sampler has made of it: an open position is reserved blank space about a token wide, an open position the source is only guessing at below the floor turns as a spin, a smear blurred past reading, an open position the source already holds a confident guess for shows that guess as a draft, a committed piece of a word whose other pieces are out is drawn as the piece it is, a complete word rolls in and builds in place where it will stand, dim, in the sampler's own order, and a run of end tokens collapses to one end mark, so the zone shortens from the tail as the model decides the length. The zone reflows as blanks, drafts and pieces become words; it is watched, never read. `lib/settle/carve.ts` derives it. The strip, one cell per position beneath the page (`lib/settle/field.ts`), is the field's compact form, for a product that wants the text zone quiet; it shows state and never text.
...
3. Text on the page never changes, moves, or reflows, with one exception: when a sentence closes, it sets. Each released word presses down by a twentieth of an em (0.05 em) and comes to rest exactly where it was written, in a ripple across the sentence (9 ms a word, at most twelve words deep) over 1.9 times the onset, while the page's ink fills its words' letterforms from the bottom up with a glint of the accent at the rising edge, and a glow of the accent around the whole passage's letterforms cools over 720 ms. After the set a word never moves, changes or reweights: its shape and its place are the ones it was written in. The carved zone after the page reflows as blanks, drafts and pieces become words: each position's width slides from what it last drew to what it draws now, and a width that shrinks glides over 2.2 times the drift, so a position gives back space more slowly than it takes it and the line moves instead of jumping. An available word is drawn in a secondary ink that clears 4.5:1 on both of the brand's grounds, computed per brand (`secondaryInk` in `lib/settle/voice.ts`); where a palette leaves no room to dim, a tint toward the accent carries the state.
...
5. Out-of-order text appears only after the page, never inside it. An open position is reserved a token's width of blank space: 4ch, where a content token renders 2.6ch wide on average across all 51 usable recordings in the surface's own face, so a word arrives into space close to the space its positions held. A smear keeps that width whatever it says. Every position of the zone is a reel: whatever it shows is a row, a change of text is a new row, and the new row rolls in from below the line, blurred, as the row it replaces rolls up and out, so at one position the reader sees the source's own sequence, the smears turning below the floor, a guess coming into focus at the floor, the guesses the source drops rolling past, and the committed word rolling in last. An open position whose guess sits below the floor draws it as a spin: the reel turning, a smear blurred past reading of the secondary ink whose letters are transparent and whose blurred shadow alone shows, at an opacity that runs from 0.15 at zero probability to 0.45 at the floor, so wherever the source is guessing the reel visibly turns and nothing below the floor is drawn as legible text. A spin is drawn only where no draft is shown and only for a piece with letters or digits, and a piece that continues a word waits for letters before it to attach to, as a draft does. A guess that stands at four or more open positions at once is the source's prior for an unknown position (the corpus prior, or a word out of the prompt), and it stays the prior while it stands at two or more; the prior is drawn as blank space, so the reel turns only where the source is guessing something about the position. An open position the source holds a confident guess for draws that guess as a draft: the source's own provisional argmax at that instant, in a ghost of the secondary ink whose opacity and sharpness follow its probability, and when the source changes its mind the reel turns, so the guess it drops rolls away above the line as the new one rolls in beneath it. A committed piece of a word that is not complete is drawn as the piece it is, because its letters are facts. A word is written where it will stand, in the secondary ink, only after every piece of it and its boundaries have committed; it rolls in last of all and stops the way a casino reel stops, rolling up from 0.9 em below the line, overshooting to 0.075 em above it, dropping back to 0.028 em below it and coming to rest, glowing in the accent as it stops, or landing where it is with the same bounce and no roll when its own draft or piece was already standing there; and it builds in place: each letter comes into focus from a blur on its own beat, 460 ms a letter with a 34 ms delay per rank, in two passes (2.2 px of blur to 1 px, a hold, then sharp), in an order that runs from the middle of the word outward with the two sides alternating, while its width slides from the width its positions actually held to its own. The letters stand in the secondary ink at full opacity on an unchanged ground from the first frame, so a written word clears the contrast floor at every moment and the softness is the blur alone. Words written at the same instant build at the same instant, in whatever places the sampler committed them, which is what a sampler filling positions in parallel looks like. A line break, once committed or confidently guessed, is drawn as a break, so the message's shape, its length, its paragraphs, its list, arrives before its words; the zone shortens from the tail as the length is decided. Nothing travels across the answer.
...
10. Reduced motion removes the mark's breath, the cell bloom, the onset, the reel's roll, the word's bounce, the word's build, the drafts' lift, the draft's reconsidering and its breath, the refocus a cell plays when it changes lines, and the set's press, glint and glow; rows stand where they are, a smear stands where it is at its weight, the page's ink arrives at once when a sentence closes, and a written word and its letters are drawn sharp at once. It changes nothing else, and it changes nothing about what is drawn or when.
...
| `onset` | 0 to 240 ms | the clock of the set (the ink's rise runs 1.9 times the onset) and of every width slide in the zone | the set's press is a twentieth of an em and ends where it began; never a blur on the page; zero under reduced motion |
| `tempo` | 0.7 to 1.4 | the breath period of the margin mark while receiving (2400 ms base) | rest at every terminal state |
| `grain` | 0 to 1 | the contrast between open cells and committed cells, and how far the secondary ink sits from the page's | available words at least 4.5:1 on both of the brand's grounds (`secondaryInk`); open cells visible |

The accent hue, the faces, and the radius stay in `BrandTokens`. The `preview` switch (forming text on or off) is a product setting rather than a voice token: a product that wants Margin's stillness turns it off and pays Margin's hold.
...
Beside the drafts, from the same script and in the same traces, is `spins`: one list per denoising step of `[position, piece, probability]` entries, recorded whenever an open position's provisional argmax changes at all, at any probability, with the first step recording every position and a commitment at a position ending its spin (`data/traces/README.md`). The reducer keeps them beside the drafts as a `spin` event, on the drafts' terms: a spin changes nothing the reader can count on, one at a committed position is ignored, a commitment ends the spin at its position, and a completed answer holds none. The carve draws one only where no draft is shown and only for a piece with letters or digits, as a smear blurred past reading (section 5.2), so a guess below the floor is never drawn as legible text: in the recordings that guess is usually the same word at many positions at once, the corpus prior or a word out of the prompt, and what the smear reports is that the reel is turning there. A guess that stands at many open positions at once is that prior, and the reducer holds it: after every spin and every commitment it recomputes the prior into `SettleState.prior`, where a guess becomes the prior at four or more open positions (`PRIOR_ENTER` in `lib/settle/reader.ts`) and stays the prior while it stands at two or more (`PRIOR_STAY`), compared without its spacing or case, with a commitment removing its position from the count. The carve reads the prior from the state and draws it as blank space; a guess specific to its position spins. The hysteresis is what keeps a count that wavers across the line from switching a field of smears on and off between steps, and a test holds it (`tests/settle/carve.test.ts`). The statistics in `data/traces/derived/drafts.json` are unchanged by the channel, because they count the drafts the surface shows at the display floor.
...
- A draft is drawn in a ghost of the secondary ink whose opacity and sharpness follow its probability: a CSS variable `--sure` runs from the floor to certainty, its opacity is 0.3 plus 0.5 times that, and its blur runs from 1.5 px at the floor to sharp at certainty, both over 460 ms, so a draft comes into focus in visible steps as the model's confidence rises across the steps.
...
- When the source changes its mind at a position, the reel turns: the guess it drops rolls up and out, blurred, as the new one rolls in beneath it, once per change.
- After a draft's letters reconsider they breathe: a slow alternation between full presence and 0.78 over 4.2 s, offset by 530 ms per position, so the unresolved part of the answer is never still and never blinks.
- When a word is written, the drafts within three positions on either side of it lift for 640 ms, brighter and sharper with a slight rise, and then return to their own probability: the recorded neighbor lift of this section, drawn.
- A draft never reaches the page, is never inside the prefix, and is never drawn as a committed word.

Authored here: the display floor, the hysteresis, the attach rule, the ghost's ramp, the smear's blur and its weights, the reconsidering, the breath, and the lift's reach and duration. Recorded: the drafts and the spins, their probabilities, their changes, and the neighbor lift the drafts' lift draws.
...
| `concept` | the wait as a product moment | what the surface opens and none of it measured: the shape of the message before its words, a draft a reader can watch become the answer, a tick under the thumb, a slot for a tip or a card, and time on app with the labor illusion as its guardrail. |
...
components/settle/  settle-answer.tsx   the product component: page, carved zone, field, margin
...
- The set: a passage arrives on the page once, whole, its opacity running from 0 to 1 over `onset` ms on an exponential ease-out while the forming register gives way to the page register over the same span. When its sentence closes, the passage sets: each released word presses down by 0.05 em and comes to rest exactly where it was written, in a ripple across the sentence (9 ms a word, at most twelve words deep) over 1.9 times the onset (380 ms where the onset is 200 ms), while the page's ink fills its letterforms from the bottom up with a glint of the accent at the rising edge (a thin band, the accent at 60 percent mixed into the page's ink, 6 percent of the gradient's height, between the page's ink below and the secondary ink above), and a glow of the accent around the whole passage's letterforms (a drop shadow of 0.32 em at 55 percent) cools over 720 ms. A phone that can tick, ticks, for 12 ms. The page's kerning and ligatures are off: a word in the zone is built from one span a letter and on the page it is one run of text, and without kerning and ligatures the two measure the same, so a sentence moving to the page moves nothing (the largest movement of any legible cell at a passage release is 0.2 px, measured). Purpose: the one event on the page worth feeling is a sentence closing, and it is the one movement the page makes; the press and the glow are over within the set, and the word ends exactly where it began. None under reduced motion, where the page's ink arrives at once.
...
- The reel: every position of the zone is one element, keyed by position, whatever it shows: reserved space, a guess below the floor, a draft, a committed piece, or a written word. Whatever it shows is a row, and a change of text is a new row. The new row rolls in from 0.9 em below the line, blurred by 3 px, and comes to rest over 460 ms on the exponential ease-out; the row it replaces, the same element it always was, rolls on up and out to 0.95 em above with 2.5 px of blur, fading over 240 ms, and is let go after 520 ms. Rows are transitions, so a guess that changes again mid-roll turns smoothly from where it is. A cell is clipped 0.14 em beyond its box vertically and 0.3 em sideways, so a row emerges from its cell and a glow is not cut. Below the floor the rows are smears (the spin, below), so the reel turns wherever the source is guessing. Purpose: at one position the reader sees the source's own sequence, the smears turning below the floor, a guess coming into focus at the floor, the guesses the source drops rolling past, and the committed word rolling in last; the order and the pace are the recording's, and the reel adds the roll. None under reduced motion, where rows stand where they are.
- The spin: an open position whose guess sits below the floor draws it as a smear blurred past reading of the secondary ink. The letters are transparent and only their blurred shadow shows, a text shadow of 0.28 em, at an opacity from 0.15 at zero probability to 0.45 at the floor (0.15 plus 0.3 times the probability's share of the floor, 0.25). A smear is drawn only where no draft is shown, only for a piece with letters or digits, and only where a piece continuing a word has letters before it to attach to; the end spelling, a special token, a break, whitespace and bare punctuation draw nothing, so the message's shape is still carved from confident guesses alone. The same guess comes into focus as a draft once it clears the floor. Purpose: the reel turns everywhere the source is guessing, and the word it turns on is not the information; in the recordings that word is usually the same one at many positions at once, the corpus prior or the prompt's own word, so the turning is drawn and the word stays blurred past reading. The prior is blank: a guess that stands at four or more open positions at once draws nothing, and it goes on drawing nothing while it stands at two or more. A smear's rows turn on a longer, softer roll than a word's (from 0.55 em below, over 620 ms, on the strong ease-out). Under reduced motion a smear stands where it is at its weight.
- The build: when every piece of a word and its boundaries are in, the word rolls in and builds in place. It stops the way a casino reel stops, rolling in from 0.9 em below the line over 640 ms, overshooting to 0.075 em above the line, dropping back to 0.028 em below it and coming to rest, glowing in the accent as it stops; a word whose text was already standing at its position as its own draft or piece lands where it is with the same bounce and no roll, over 520 ms (0.06 em down, 0.05 em up, 0.02 em down, rest), while the cell's blur clears over 460 ms as the ghost becomes ink. The roll and the bounce are transform and blur only. Each letter comes into focus from a blur on its own beat, 460 ms a letter with a 34 ms delay per rank, in two passes (2.2 px of blur to 1 px, a hold, then sharp), in an order that runs from the middle of the word outward with the two sides alternating (`buildOrder` in the component), so no letter order is left to right. The letters are in the secondary ink at full opacity on an unchanged ground from the first frame, so a written word clears the contrast floor at every moment and the softness is the blur alone. Words written at the same instant build at the same instant, in whatever places the sampler committed them, which is what a sampler filling positions in parallel looks like. The build starts the moment the word is written, after its commitment, and no word waits for the one before it. Purpose: the one moment in the zone worth marking is the moment a guess becomes a fact, and the word takes shape where it already stands.
- The width slide: an open position holds 4ch of blank space (`--settle-slot-w`), a token's width. Across all 51 usable recordings a content token renders 2.6ch wide on average in the surface's own face, so at the 2.3ch the zone reserved before, a landing word was usually wider than the space its positions held and pushed the line forward; at 4ch it arrives into space close to what it had. A position's width slides from the width it last drew (blank, draft, or pieces) to the width it draws now over the drift: at least 520 ms, or the onset where that is longer, varied by position from 0.9 to 1.1 times so the line breathes rather than moving in step, on the strong ease-out. A width that shrinks glides over 2.2 times the drift, so a position gives back space more slowly than it takes it and a guess that comes and goes within a few steps barely moves the line. A change that lands mid-slide turns from the width on screen (the slide once restarted from the width it began at, which made the words jump). A smear takes no part in the line's width at all: its rows are out of the flow, centered in the position and clipped at its sides, and the cell sits on the line the way a blank does, so the reel below the floor turns without moving anything. The surface remembers each position's last drawn width. Purpose: the line moves instead of jumping.
- The refocus: inline flow moves a cell to another line in one frame. The surface compares each position's top after every render, and where a cell has moved by ten pixels or more it plays a 320 ms blur in on it (`settle-refocus`, from 2.4 px to the cell's own blur, with no change of color, so a committed word holds its contrast at every frame); a cell that moves again mid-refocus restarts it. Purpose: a cell that changes lines comes back into focus where it landed instead of teleporting there. None under reduced motion.
- The draft's ramp: a guess's opacity and blur follow its probability, from the floor to certainty, and a change of guess is a turn of the reel (above). Purpose: a guess has to look like a guess, and a change of mind has to be visible as one.
- The draft's breath: after a turn of the reel, a draft's letters alternate slowly between full presence and 0.78 over 4.2 s, offset by 530 ms per position. Purpose: the unresolved part of the answer is never still and never blinks. None under reduced motion.
- The neighbors' lift: when a word is written, the drafts within three positions on either side lift for 640 ms, brighter and sharper with a slight rise, and then return to their own probability. Purpose: a commitment really does lift its neighbors' confidence (section 4.7), and this draws that where it happens. None under reduced motion.
- Nothing else moves. No entrance stagger on the page, no halo, no weight change, no exhale, and no body of any kind between the reader and the text. Nothing in the zone loops but a draft's breath. Under reduced motion the reel's rows stand where they are, a smear stands where it is at its weight, the drafts neither breathe nor lift, a written word arrives with no roll and no bounce and its letters are drawn sharp at once, a cell that changes lines is drawn where it landed with no refocus, and a sentence sets with the page's ink at once, with no press, no glint and no glow.

### 5.3 Accessibility

The page is a `region` labeled as the answer; passages are plain spans. The zone after the page is `aria-hidden`: it is the source's state drawn where the words will stand, and the status words carry its meaning. The status words live in a polite, atomic live region and change only on state changes, and never on every commit. Controls are native buttons and selects. Reduced motion is honored in CSS and in the choreography. Contrast: page text and every committed letter in the zone (a piece, a written word at every moment of its build) at least 4.5:1 on every brand, through the secondary ink's floor. A draft is exempt as incidental, provisional text: it is the source's guess drawn as a ghost on purpose, the answer never depends on it, and a reader who cannot see it loses nothing the contract promises. A row leaving the reel is exempt on the same basis: it is the ghost of a guess on its way out. A smear is exempt for a stronger reason: it is blurred past reading by design, its letters drawn as a blurred shadow so that nothing below the floor is drawn as legible text. The axe check in `tests/e2e/a11y.spec.ts` excludes drafts, smears (`.settle-zone .settle-cz[data-state="spin"]`), dropped rows (`.settle-zone .settle-cz-text[data-past]`) and the register legend's decorative samples on that basis, and holds everything else to the floor. A roll and a bounce are transform and blur alone, and the refocus a cell plays when it changes lines is a blur alone with no change of color, so committed letters stay in the secondary ink at full opacity from the first frame; and the set keeps the page's text at full contrast, since the press is a transform and the ink only darkens. `tests/e2e/reduced-motion-full.spec.ts` covers the passage as well as the zone.
...
9. A written word builds only after its commitment, starting the moment it is written.
10. Nothing travels across the answer, and a break is drawn only for a committed or confidently guessed line break. A spin is drawn only for a piece with letters or digits, never where a draft is shown, never at a committed position, and never as legible text.
11. A reel's rows at a position are the recording's guesses at that position, in the recording's order; a dropped guess is drawn as a ghost and never as a word; and the set is over within its own duration, with every released word ending where it began.
12. A guess that stands at four or more open positions at once is drawn as blank, and stays blank while it stands at two or more, whatever its spacing or case; a commitment removes its position from the count (`PRIOR_ENTER` and `PRIOR_STAY` in `lib/settle/reader.ts`, `tests/settle/carve.test.ts`).
13. A sentence moving to the page moves no legible cell: with the page's kerning and ligatures off, the largest movement of any legible cell at a passage release is 0.2 px.
