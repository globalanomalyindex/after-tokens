...
**The field** is where the reader watches. It is the sampler's own field of positions, drawn as one line of cells directly beneath the page. Each cell is one token position of the request's bound. A cell is open, committed, end, held, forming, or released. The field shows exactly what the sampler has done: where it has committed, where the holes are that keep the page waiting, and, once end tokens settle, how long the answer is going to be. It shows state and never text.
...
3. Text is appended only at the end of the contiguous prefix. Nothing visible changes, moves, or reflows, except that forming text brightens into the page.
...
5. Out-of-order state appears only in the field, as cell state, never as text.
6. Length is claimed only when the contiguous prefix reaches a committed end token. Before that the field shows committed end cells as what they are: positions the sampler has marked as end, collapsed into one end cell with a count.
