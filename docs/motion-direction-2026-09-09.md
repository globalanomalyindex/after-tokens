...
The Pages workflow disables only the disposable runner's unused Google Chrome APT repository, guarded by its filename and URL. Two release attempts encountered a hash mismatch in that unrelated repository during Playwright's global package-index update. The workflow still installs Playwright's Chromium with its system dependencies and runs every configured Chromium browser check; it does not bypass package integrity verification. This is deployment maintenance, not a change to the renderer or the research result. [Playwright dependency installation](https://github.com/microsoft/playwright/blob/v1.60.0/packages/playwright-core/src/server/registry/dependencies.ts#L81-L113), [GitHub runner Chrome cleanup](https://github.com/actions/runner-images/blob/main/images/ubuntu/scripts/build/install-google-chrome.sh#L31-L38).
