...
import { MATERIAL, REPORT, implementationSnapshot, fingerprint, hash } from './archive-responsive-skeleton-motion.mjs'

const root = process.cwd(), input = 'output/playwright/responsive-skeleton-e2e-2026-09-12.json'
const directory = 'data/experiments/responsive-skeleton-browser-2026-09-12'
const reportFile = 'docs/responsive-skeleton-browser-validation-2026-09-12.json'
const bytes = await readFile(path.join(root, input)), raw = JSON.parse(bytes)
const provenanceFile = 'output/playwright/responsive-skeleton-e2e-provenance-2026-09-12.json'
const provenanceBytes = await readFile(path.join(root, provenanceFile)), provenance = JSON.parse(provenanceBytes)
assert.equal(raw.stats.expected, 69); assert.equal(raw.stats.unexpected, 0); assert.equal(raw.stats.flaky, 0); assert.equal(raw.stats.skipped, 0)
...
for (const test of tests) for (const attachment of test.results[0].attachments ?? []) {
  if (!['v6-forced-fit-result', 'v6-earlier-reading-result'].includes(attachment.name)) continue
  const body = attachment.body ? Buffer.from(attachment.body, 'base64') : await readFile(attachment.path)
  const result = JSON.parse(body)
  if (attachment.name === 'v6-earlier-reading-result') {
...
const testFiles = await Promise.all(['a11y', 'ambient-answer', 'reduced-motion-full', 'settle-motion', 'snapshot-study'].map(async (name) => {
