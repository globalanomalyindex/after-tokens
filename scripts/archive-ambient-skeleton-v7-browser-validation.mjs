...
import { MATERIAL, REPORT, implementationSnapshot, fingerprint, hash } from './archive-ambient-skeleton-v7-motion.mjs'

const root = process.cwd(), input = 'output/playwright/ambient-skeleton-v7-e2e-2026-09-12.json'
const directory = 'data/experiments/ambient-skeleton-v7-browser-2026-09-12'
const reportFile = 'docs/ambient-skeleton-v7-browser-validation-2026-09-12.json'
const bytes = await readFile(path.join(root, input)), raw = JSON.parse(bytes)
const provenanceFile = 'output/playwright/ambient-skeleton-v7-e2e-provenance-2026-09-12.json'
...
for (const test of tests) for (const attachment of test.results[0].attachments ?? []) {
  if (!['v7-forced-fit-result', 'v7-earlier-reading-result'].includes(attachment.name)) continue
  const body = attachment.body ? Buffer.from(attachment.body, 'base64') : await readFile(attachment.path)
  const result = JSON.parse(body)
  if (attachment.name === 'v7-earlier-reading-result') {
