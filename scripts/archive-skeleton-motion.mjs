...
export const SKELETON_MATERIAL = 'solid-rounded-skeleton-v1'
export const SKELETON_ARCHIVE = 'data/experiments/skeleton-motion-2026-09-09'
export const SKELETON_ARTIFACTS = 'output/playwright/skeleton-motion-2026-09-09'
export const SKELETON_REPORT = 'docs/skeleton-motion-validation-2026-09-09.json'
export const skeletonHash = (bytes) => createHash('sha256').update(bytes).digest('hex')
const implementationFiles = [
  'app/ambient-composition.css', 'app/globals.css',
  'components/settle/settle-answer.tsx', 'components/settle/ambient-composition.tsx',
  'components/settle/ambient-study.tsx', 'components/settle/use-replay.ts',
  'lib/brand/brands.ts', 'lib/brand/provider.tsx', 'lib/settle/voice.ts',
  'lib/settle/experimental-recordings.ts', 'lib/settle/reader.ts', 'lib/settle/replay.ts',
  'scripts/measure-skeleton-motion.mjs', 'scripts/archive-skeleton-motion.mjs',
]
...
export function assertSkeletonCompatibility(previous, current) {
  for (const key of ['material', 'fingerprint', 'playbackScale', 'cyclePeriodMs', 'schemaVersion']) {
...
export async function archiveSkeletonMotion(report, root, artifacts = path.join(root, SKELETON_ARTIFACTS)) {
  ...
  const pendingVideos = []
  for (const [name, destination] of [['390-reshape', 'skeleton-answer-mobile.webm'], ['390-reshape-sky', 'skeleton-answer-long.webm']]) {
  ...
  const manifest = {
    ...
    playbackScale: report.playbackScale, cyclePeriodMs: report.cyclePeriodMs,
    ...
    provenance: 'Implementation/source fingerprints captured before measurement and verified unchanged before archive. Per-case measurement dates are retained. This is a separate solid-bar archive; the earlier feathered treatment is not overwritten.',
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!path.basename(reportFile).startsWith('skeleton-')) throw Error('Refusing to overwrite a report outside the skeleton namespace')
