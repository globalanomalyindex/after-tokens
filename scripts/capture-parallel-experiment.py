...
Each model evaluation predicts all open positions, and the sampler commits
four open positions in one operation, by confidence or seeded random scores. Provisional deltas
...
def main() -> None:
    ...
    manifest = {
        ...
        "limitations": [
            f"{len(args.prompts)} fixed prompt(s) from one small model and one sampler; no claims of representative answer quality or reader benefit.",
