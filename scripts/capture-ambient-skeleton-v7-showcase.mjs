...
import { capturedClock } from './measure-ambient-skeleton-v7-motion.mjs'
import { MATERIAL as SKELETON_MATERIAL, REPORT as SKELETON_REPORT, hash as skeletonHash, implementationSnapshot as skeletonImplementationSnapshot } from './archive-ambient-skeleton-v7-motion.mjs'

// These passive recordings are presentation artifacts, separate from the
// instrumented geometry observations and their raw data archives.
...
const manifestPath = 'docs/ambient-skeleton-v7-showcase-capture-2026-09-12.json'
const artifacts = 'output/playwright/ambient-skeleton-v7-showcase-2026-09-12'
const cases = [
...
].map((item) => ({ ...item, destination: `public/study/ambient-skeleton-v7-showcase-${item.name}.webm` }))
for (const file of [manifestPath, ...cases.flatMap((item) => [item.destination, item.destination.replace(/\.webm$/, '.png')])]) {
...
const browser = await chromium.launch(), recordings = [], pending = [], posters = []
try {
  ...
  for (const item of cases) {
    ...
    const clock = capturedClock(trace, .5), viewport = item.name === 'mobile' ? { width: 390, height: 844 } : { width: 1380, height: 900 }, recordedAt = new Date().toISOString()
    ...
    try {
      ...
      if (viewport.width < 768) await study.getByRole('radiogroup', { name: 'motion study', exact: true }).getByRole('radio', { name: 'reshape', exact: true }).click()
  ...
  for (const item of cases) {
    const viewport = item.name === 'mobile' ? { width: 390, height: 844 } : { width: 1380, height: 900 }
    ...
    try {
      ...
      const file = `public/study/ambient-skeleton-v7-showcase-${item.name}.png`, bytes = await page.screenshot()
      pending.push({ file, bytes })
      posters.push({ name: item.name, file, sha256: skeletonHash(bytes), bytes: bytes.length, viewport, condition: 'reshape', method: 'Separate browser viewport screenshot approximately1300ms after replay; source clock0.5×, decorative clock unchanged. Not a frame extracted from the passive video.' })
    } finally { await context.close() }
  }
  ...
  const manifest = {
    ...
    browser: 'Chromium', browserVersion: browser.version(), url, recordings, posters,
