...
import { capturedClock } from './measure-responsive-skeleton-motion.mjs'
import { MATERIAL as SKELETON_MATERIAL, REPORT as SKELETON_REPORT, hash as skeletonHash, implementationSnapshot as skeletonImplementationSnapshot } from './archive-responsive-skeleton-motion.mjs'
...
const manifestPath = 'docs/responsive-skeleton-showcase-capture-2026-09-12.json'
const artifacts = 'output/playwright/responsive-skeleton-showcase-2026-09-12'
const cases = [
...
].map((item) => ({ ...item, destination: `public/study/responsive-skeleton-showcase-${item.name}.webm` }))
...
try {
  ...
  for (const item of cases) {
    ...
    try {
      ...
      const file = `public/study/responsive-skeleton-showcase-${item.name}.png`, bytes = await page.screenshot()
