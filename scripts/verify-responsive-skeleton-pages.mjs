...
import { capturedClock, observeResponsive, summarizeResponsive } from './measure-responsive-skeleton-motion.mjs'
import { MATERIAL as SKELETON_MATERIAL, REPORT, implementationSnapshot, fingerprint } from './archive-responsive-skeleton-motion.mjs'

// Served-build smoke check, with the same strict v6 material/finality/opacity/rest oracle as
...
const reportPath = process.env.VERIFY_REPORT ?? 'output/playwright/responsive-skeleton-pages-verification.json'
...
try {
  ...
  page.on('requestfailed', (request) => {
    ...
    const isShowcase = ['mobile', 'long'].some((id) => request.url() === new URL(`study/responsive-skeleton-showcase-${id}.webm`, base).href)
  ...
  for (const fixture of fixtures) {
    ...
    const raw = await surface.evaluate(observeResponsive, { expectedText: fixture.trace.answer, sourceId: fixture.trace.id, clock, material: SKELETON_MATERIAL, playbackScale: 1 })
    observations.push({ fixture: fixture.file, fixtureSha256: fixture.sha256, ...summarizeResponsive(raw, { name: fixture.trace.id, condition: 'reshape', viewport: { width: 390, height: 844 }, source: fixture.trace.id }) })
  }
  ...
  const showcaseVideos = await verifyVideos(['responsive-skeleton-showcase-mobile.webm', 'responsive-skeleton-showcase-long.webm'])
  const showcasePlayback = [], showcasePosters = []
  for (const id of ['mobile', 'long']) {
    const video = page.locator(`[data-showcase-video="${id}"]`)
    await expect(video).toHaveAttribute('src', `/after-tokens/study/responsive-skeleton-showcase-${id}.webm`)
    const posterName = `responsive-skeleton-showcase-${id}.png`
...
7. Add the two parts: 36 + 3 = 39.`
  ...
  const report = { implementationFingerprint: measured.fingerprint, measuredReport: REPORT, verifiedAt: new Date().toISOString(), url: base, browser: browser.version(), status: response.status(), title: await page.title(), canonical, material: SKELETON_MATERIAL, viewport: { width: 390, height: 844 }, assetCount: assets.length, assetPrefix: '/after-tokens/', assets, assetFiles,
    observationMethod: 'MutationObserver installed before replay plus requestAnimationFrame; v6 material/source-finality/full-opacity/rest oracle at original source speed, with actual captured-cell transfer and no old renderer. Local full-word rectangles must fit the visible final frame; this is not a raster or whole-screen occlusion audit.',
