...
import { archiveAmbientV7, fingerprint, implementationSnapshot, hash, MATERIAL, REPORT, ARTIFACTS } from './archive-ambient-skeleton-v7-motion.mjs'
...
export async function observeAmbientV7(root, input) {
      ...
      const tick = (atMs) => {
        try {
          ...
          const rows = [...root.querySelectorAll('.ambient-composition__bar')].map((node) => {
            ...
            const pills = node.dataset.shown === 'true' ? [...node.querySelectorAll('.ambient-composition__presence')].map((pill) => {
              ...
              return { identity: identity(pill, cells), x: r.left - box.left, y: r.top - box.top, width: r.width, height: r.height, opacity: Number(pc.opacity),
                filter: ic.filter, backgroundImage: ic.backgroundImage, maskImage: ic.maskImage, shadow: ic.boxShadow, radius: ic.borderTopLeftRadius, target: { x: pill.style.left, width: pill.style.width, opacity: pill.style.opacity } }
            }) : []
            return { identity: identity(node, frames), shown: node.dataset.shown === 'true', kind: node.dataset.kind, episode: Number(node.dataset.scoreEpisode), cueAtMs: Number(node.dataset.scoreCue), gesture: node.dataset.scoreGesture, transitionMs: parseFloat(css.getPropertyValue('--shape-duration')), ...rect(node, frame), opacity: Number(css.opacity), period: css.animationDuration, pills }
          })
          ...
          samples.push({ atMs, elapsedMs: atMs - startedAt, sourceElapsedMs: Number(study.dataset.elapsedMs), sourceAtMs: Number(root.dataset.sourceAtMs),
            ...
            activityMs: Number(composition?.dataset.activityMs ?? 0), scoreMs: Number(composition?.dataset.scoreMs ?? 0), geometrySource: composition?.dataset.geometrySource ?? null, rows,
...
export function summarizeAmbientV7(raw, meta = {}) {
  ...
  fail(loading.every((sample) => sample.rows.length === 5 && sample.rows.every((row, index) => row.identity === initialRows[index])), 'waiting rows remounted')
  fail(rowCounts.every((count) => count === 5), 'waiting row budget invalid')
  fail(loading.every((sample) => Math.abs(sample.frame.height - sample.lineHeight * 5) < 1), 'waiting frame outside typographic bounds')
  fail(range(loading.map((sample) => sample.frame.height)) < .05, 'source content changed the waiting frame')
  fail(loading.every((sample) => sample.geometrySource === 'decorative-score'), 'waiting geometry lost its source-independent contract')
  ...
  const episodes = [], targetByRow = new Map(), restBaselines = new Map()
  let maximumSimultaneousTargetChanges = 0, geometricRestSamples = 0, postEpisodeRestSamples = 0, maximumRestCellDriftPx = 0
  for (const [index, sample] of loading.entries()) {
    let changedRows = 0
    for (const [rowIndex, row] of sample.rows.entries()) {
      const targets = JSON.stringify(row.pills.map((pill) => pill.target))
      const previous = targetByRow.get(rowIndex)
      fail(Number.isFinite(row.episode) && Number.isFinite(row.cueAtMs) && Number.isFinite(row.transitionMs) && row.transitionMs >= 900 && row.transitionMs <= 1300, 'score timing metadata invalid')
      if (previous && targets !== previous.targets) {
        changedRows++
        fail(row.episode > previous.episode && row.cueAtMs > previous.cueAtMs && rowIndex !== 0, 'cell targets changed without a new eligible score episode')
        episodes.push({ row: rowIndex, episode: row.episode, cueAtMs: row.cueAtMs, observedAtScoreMs: sample.scoreMs, transitionMs: row.transitionMs, gesture: row.gesture })
      }
      targetByRow.set(rowIndex, { targets, episode: row.episode, cueAtMs: row.cueAtMs })
    }
    maximumSimultaneousTargetChanges = Math.max(maximumSimultaneousTargetChanges, changedRows)
    const previousSample = loading[index - 1]
    const atRest = previousSample && sample.scoreMs > 1000 && sample.rows.every((row) => row.episode < 0 || previousSample.scoreMs >= row.cueAtMs + row.transitionMs + 200)
    if (atRest) {
      geometricRestSamples++
      if (sample.rows.some((row) => row.episode >= 0)) postEpisodeRestSamples++
      const restKey = sample.rows.map((row) => row.episode).join(':')
      if (!restBaselines.has(restKey)) restBaselines.set(restKey, sample.rows)
      const baseline = restBaselines.get(restKey)
      for (const [rowIndex, row] of sample.rows.entries()) for (const [cellIndex, cell] of row.pills.entries()) {
        const previous = baseline[rowIndex].pills[cellIndex]
        maximumRestCellDriftPx = Math.max(maximumRestCellDriftPx, Math.abs(cell.x - previous.x), Math.abs(cell.width - previous.width))
      }
    }
  }
  fail(maximumSimultaneousTargetChanges <= 1, 'multiple rows received simultaneous decorative targets')
  fail(maximumRestCellDriftPx < .05, 'cell geometry did not rest between episodes')
  if (reshaping) {
    ...
    fail(range(loading.map((sample) => sample.activityMs)) > 1000, 'decorative activity clock did not advance')
    fail(episodes.length > 0 && geometricRestSamples > 10 && postEpisodeRestSamples > 10, 'score episodes or geometric rests were not observed')
    fail(cellMotionPx > .1, 'decorative cells did not change geometry')
  }
  ...
  const summary = { ...meta, material: MATERIAL, measuredAt: raw.measuredAt, source: raw.sourceId, frames: raw.samples.length,
    ...
    scoreEpisodes: episodes, maximumSimultaneousTargetChanges, geometricRestSamples, postEpisodeRestSamples, maximumRestCellDriftPx, sourceIndependentWaiting: true, maximumGlimmerPhaseSpreadMs: glimmerSpread, frameGapP95Ms: percentile(gaps, .95), maximumFrameGapMs: maximum(gaps), guards: { passed: failures.length === 0, failures } }
  if (failures.length) throw Object.assign(Error(`${meta.name ?? 'observation'}: ${failures.join('; ')}`), { summary })
  return summary
}

export async function measureAmbientV7() {
  ...
  try {
    for (const item of cases) {
      ...
      try {
        ...
        const raw = await surface.evaluate(observeAmbientV7, { ...contract, expectedText: fixture.trace.answer, sourceId: fixture.trace.id, clock })
        await writeFile(path.join(artifacts, `${name}-raw.json`), JSON.stringify(raw))
        if (errors.length) throw Error(`Browser errors: ${errors.join('; ')}`)
        const summary = summarizeAmbientV7(raw, { name, viewport, condition: item.condition })
    ...
    const report = { ...contract, measurementStartedAt: started, measuredAt: new Date().toISOString(), url, browser: 'Chromium', browserVersion: browser.version(),
      ...
      caveats: ['No physical device FPS or reader-benefit evidence.', 'These finite observations cover some decorative episodes and rests, not a complete10.8s round or infinite novelty.', 'Instrumented rAF gaps are diagnostic only, not rendering-cost comparisons.', 'Source policies, opacity handover and size fitting have distinct timing costs.', 'Capsule division, seeded decorative episodes, breathing, glimmer and handover are a bundled treatment.', 'Opacity1 can precede the end of the280ms movement/transfer; readiness is measured separately.', 'Waiting geometry is source-independent; only already eligible text can resize the reading surface. Five decorative rows are not a future layout or percent complete.'],
    }
    const archived = await archiveAmbientV7(report, root)
    await writeFile(path.join(root, REPORT), JSON.stringify(archived, null, 2) + '\n')
  } finally { await browser.close() }
}
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) await measureAmbientV7()
